...
Type
  ...
  TAccountsGridUpdateThread = Class(TABEYThread)
  ...
  TOperationsGridUpdateThread = Class(TABEYThread)
  ...
  TBlockChainGridUpdateThread = Class(TABEYThread)
...
Const
  CT_TBlockChainData_NUL : TBlockChainData = (Block:0;Timestamp:0;BlockProtocolVersion:0;BlockProtocolAvailable:0;OperationsCount:-1;Volume:-1;Reward:0;Fee:0;Target:0;HashRateTargetHs:0;HashRateHs:0;HashRateTargetKhs:0;HashRateKhs:0;MinerPayload:Nil;PoW:Nil;VaultHash:Nil;AccumulatedWork:0;TimeAverage200:0;TimeAverage150:0;TimeAverage100:0;TimeAverage75:0;TimeAverage50:0;TimeAverage25:0;TimeAverage10:0);
...
uses
  ...
  UFRMPayloadDecoder, ULog, UFRMWallet;
  ...
  try
      if (Assigned(FAccountsGridFilter.OrderedAccountsKeyList)) then begin
        ...
        while (Not Terminated) and (i<FAccountsGridFilter.OrderedAccountsKeyList.Count)
          ...
          LNode.bank.Vault.StartThreadSafe;
          ...
          Try
            ...
            for j := 0 to l.Count - 1 do begin
              LAccount := LNode.Bank.Vault.Account(l.Get(j));
            ...
            LNode.Bank.Vault.EndThreadSave;
          end;
          inc(i);
        end;
      end else begin
        c := 0;
        while (c<LNode.Bank.Vault.AccountsCount) and (Not Terminated) do begin
          LAccount := LNode.Bank.Vault.Account(c);
          if (LAccount.balance>=FAccountsGridFilter.MinBalance) And ((FAccountsGridFilter.MaxBalance<0) Or (LAccount.balance<=FAccountsGridFilter.MaxBalance)) then begin
            FProcessedList.Add(LAccount.account);
            FBalance := FBalance + LAccount.balance;
          end;
...
Const CT_ColumnHeader : Array[TAccountColumnType] Of String =
  ('ID','Key','Balance','Updated','Transactions',' ','Name','Category','Price');
...
begin
  ...
  FColumns[0].ColumnType := act_updated_state;
  FColumns[0].width := 25;
  FColumns[1].ColumnType := act_account_number;
  FColumns[1].width := 75;
  FColumns[2].ColumnType := act_name;
  FColumns[2].width := 180;
  FColumns[3].ColumnType := act_type;
  FColumns[3].width := 100;
  FColumns[4].ColumnType := act_balance;
  FColumns[4].width := 160;
  FColumns[5].ColumnType := act_n_operation;
  FColumns[5].width := 100;
  FColumns[6].ColumnType := act_saleprice;
  FColumns[6].width := 160;

  if (Assigned(Application.MainForm) and Assigned(Application.MainForm as TFRMWallet)) then
    (Application.MainForm as TFRMWallet).ResizeColumns; // resize the columns here
...
Var C : TAccountColumn;
  ...
  account : TAccount;
  ndiff : Cardinal;
  LNodeBlocksCount,LNodeAccountsCount, imgWidth : Integer;
begin
  if Not Assigned(Node) then exit;
  ...
  imgWidth := FRMWallet.imgUserUnknown.Picture.Graphic.Width;
      ...
      case C.ColumnType of
        ...
        act_balance : Begin
          if ndiff=0 then begin
            ...
            s := '[ '+TAccountComp.FormatMoney(account.balance)+' ABEY ]';
          end else begin
            s := TAccountComp.FormatMoney(account.balance)+' ABEY';
        ...
        act_updated_state : Begin
          ...
          // first, the icon
          if (account.account_type = ACCOUNT_USER) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgUser.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_CONTRACT) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgContract.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_MERCHANT) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgMerchant.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_NONPROFIT) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgNonProfit.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_MINER) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgMiner.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_MININGPOOL) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgMiningPool.Picture.Graphic)
          else
          if (account.account_type = ACCOUNT_ACADEMIA) then
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgAcademia.Picture.Graphic)
          else
            // unknown
            DrawGrid.Canvas.Draw(Rect.Left,Rect.Top,FRMWallet.imgUserUnknown.Picture.Graphic);
          ...
          DrawGrid.Canvas.FillRect(Rect.Left+imgWidth, Rect.Top+0, Rect.Left+imgWidth+5, Rect.Top+5);
          //DrawGrid.Canvas.Ellipse(Rect.Left+1,Rect.Top+1,Rect.Right-1,Rect.Bottom-1);
        End;
        act_name : Begin
        ...
        act_type : Begin // 30.04.2020 --> categories from 1 to 1024 are reserved
        ...
        act_saleprice : Begin
          if TAccountComp.IsAccountForSale(account.accountInfo) then begin
            // Show price for sale
            s := TAccountComp.FormatMoney(account.accountInfo.price)+' ABEY';
...
begin
  if Assigned(Node) then begin
    case FAccountsGridDatasource of
      acds_Node: FAccountsBalance := Node.Bank.Vault.TotalBalance;
      acds_InternalList: begin
        FAccountsBalance := 0;
        for i := 0 to FAccountsList.Count - 1 do begin
          LAcc := Node.Bank.Vault.Account( FAccountsList.Get(i) );
      ...
      try
        for i := LLockedMempool.Count - 1 downto 0 do begin
          ...
          If TABEYOperation.OperationToOperationResume(0,Op,True,Op.SignerAccount,OPR) then begin
            ...
            OPR.Balance := LLockedMempool.VaultTransaction.Account(Op.SignerAccount).balance;
            AList.Add(OPR);
          end;
        end;
      finally
          ...
          while (bstart<=bend) and (Not Terminated) do begin
            ...
            if (ANode.Bank.Storage.LoadBlockChainBlock(opc,bend)) then begin
              ...
              for i := opc.Count - 1 downto 0 do begin
                if TABEYOperation.OperationToOperationResume(bend,opc.Operation[i],True,opc.Operation[i].SignerAccount,opr) then begin
                  opr.NOpInsideBlock := i;
                  opr.Block := bend;
                  opr.time := opc.OperationBlock.timestamp;
                  AList.Add(opr);
        ...
        Try
          ...
          try
            ...
            for i := list.Count - 1 downto 0 do begin
              ...
              If TABEYOperation.OperationToOperationResume(0,Op,False,FOperationsGrid.AccountNumber,OPR) then begin
                OPR.NOpInsideBlock := i;
                OPR.Block := LLockedMempool.OperationBlock.block;
                OPR.Balance := LLockedMempool.VaultTransaction.Account(FOperationsGrid.AccountNumber).balance;
                AList.Add(OPR);
              end;
            end;
          finally
...
{  DrawGrid.ColWidths[0] := 110; // Time
  DrawGrid.ColWidths[1] := 100; // Account ID
  DrawGrid.ColWidths[2] := 230; // OpType
//  DrawGrid.ColWidths[3] := 120; // Amount
  DrawGrid.ColWidths[4] := 100; // Operation Fee
  DrawGrid.ColWidths[5] := 200; // Balance
  DrawGrid.ColWidths[6] := 50; // Data/Payload
  DrawGrid.ColWidths[7] := 70; // Block
  DrawGrid.ColWidths[8] := 70; // Transaction}
  ...
  if (ARow=0) then begin
    ...
    if (ARow<=FOperationsResume.Count) then begin
      ...
      if ACol=0 then begin
        ...
        s := TAccountComp.FormatMoney(opr.Amount)+' ABEY';
        ...
        s := TAccountComp.FormatMoney(opr.Fee)+' ABEY';
        ...
        if opr.time=0 then begin
          // Pending operation... showing final balance
          DrawGrid.Canvas.Font.Color := clBlue;
          s := '[ '+TAccountComp.FormatMoney(opr.Balance)+' ABEY ]';
        end else begin
          s := TAccountComp.FormatMoney(opr.Balance)+' ABEY';
        ...
        s := '1';
        if opr.NOpInsideBlock>=0 then
          s := FloatToStrF(opr.NOpInsideBlock+1, ffNumber, 18, 0);
        Canvas_TextRect(DrawGrid.Canvas,Rect,s,State,[tfCenter,tfVerticalCenter,tfSingleLine]);
      end else begin
        s := '(???)';
        Canvas_TextRect(DrawGrid.Canvas,Rect,s,State,[tfCenter,tfVerticalCenter,tfSingleLine]);
...
begin
  ...
  try
    ...
    while (ABlockStart<=ABlockEnd) and (Not Terminated) do begin
      ...
      opb := ANode.Bank.Vault.Block(ABlockEnd).blockchainInfo;
      bcd.Block:=opb.block;
      ...
      bn := ANode.Bank.Vault.CalcBlockHashRateInHs(bcd.Block,FBlockChainGrid.HashRateAverageBlocksCount);
      try
      ...
      bcd.VaultHash := opb.initial_safe_box_hash;
      bcd.AccumulatedWork := ANode.Bank.Vault.Block(bcd.Block).AccumulatedWork;
      if (Not Terminated) then begin
...
{  DrawGrid.ColWidths[0] := 50; // Block
  ...
  DrawGrid.ColWidths[10] := 190; // Vault Hash
  ...
  if (ARow=0) then begin
    ...
    if ((ARow-1)<FBlockChainDataList.Count) then begin
      ...
      if ACol=0 then begin
        ...
        s := TCrypto.ToHexaString(bcd.VaultHash);
        Canvas_TextRect(DrawGrid.Canvas,Rect,s,State,[tfLeft,tfVerticalCenter]);
