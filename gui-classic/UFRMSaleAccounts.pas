...
begin
  ...
  Try
    ...
    for iAcc := 0 to FSenderAccounts.Count - 1 do begin
      op := Nil;
      account := FNode.Operations.VaultTransaction.Account(FSenderAccounts.Get(iAcc));
...
begin
  ...
  Try
    ...
    if SenderAccounts.Count<=0 then begin
      ...
      memoAccounts.Visible := false;
      ebSenderAccount.Visible := true;
      balance := TNode.Node.Operations.VaultTransaction.Account(SenderAccounts.Get(0)).balance;
    end else begin
      ...
      for i := 0 to SenderAccounts.Count - 1 do begin
         acc := TNode.Node.Operations.VaultTransaction.Account(SenderAccounts.Get(i));
