...
type
  ...
  private
    ...
    Function GetSelectedOperation : TABEYOperation;
...
LBL_start_sender:
    ...
    If Assigned(FSourceNode) then begin
      If (nSender<FSourceNode.Bank.AccountsCount) then begin
        n_operation := FSourceNode.Bank.Vault.Account(nSender).n_operation+1;
        amount := FSourceNode.Bank.Vault.Account(nSender).balance;
      end;
    end;
...
begin
  ...
  For i:=0 to High(mop.Data.txSenders) do begin
    j := FSourceWalletKeys.IndexOfAccountKey(FSourceNode.Bank.Vault.Account(mop.Data.txSenders[i].Account).accountInfo.accountKey);
    If (j>=0) then begin
      // Can sign
      If (Assigned(FSourceWalletKeys.Key[j].PrivateKey)) then begin
        inc(n, mop.DoSignMultiOperationSigner(SourceNode.Bank.Vault.CurrentProtocol,mop.Data.txSenders[i].Account,FSourceWalletKeys.Key[j].PrivateKey));
      end;
    end;
  end;
  For i:=0 to High(mop.Data.changesInfo) do begin
    j := FSourceWalletKeys.IndexOfAccountKey(FSourceNode.Bank.Vault.Account(mop.Data.changesInfo[i].Account).accountInfo.accountKey);
    If (j>=0) then begin
      // Can sign
      If (Assigned(FSourceWalletKeys.Key[j].PrivateKey)) then begin
        inc(n, mop.DoSignMultiOperationSigner(SourceNode.Bank.Vault.CurrentProtocol,mop.Data.changesInfo[i].Account,FSourceWalletKeys.Key[j].PrivateKey));
      end;
    end;
  end;
...
LBL_start_changer:
    ...
    If Assigned(FSourceNode) then begin
      If (nAccount<FSourceNode.Bank.AccountsCount) then begin
        n_operation := FSourceNode.Bank.Vault.Account(nAccount).n_operation+1;
        new_name:= TEncoding.ANSI.GetString( FSourceNode.Bank.Vault.Account(nAccount).name );
        new_type:= FSourceNode.Bank.Vault.Account(nAccount).account_type;
        new_account_key := FSourceNode.Bank.Vault.Account(nAccount).accountInfo.accountKey;
      end;
    end;
    ...
    Repeat
      ...
      //aux := LowerCase(aux);
    Until (aux='') Or (TABEYVault.ValidAccountName(TEncoding.ANSI.GetBytes(aux),errors));
    new_name := aux;
...
begin
  ...
  If lbOperations.ItemIndex>=0 then begin
    ...
    else op := TABEYOperation( lbOperations.Items.Objects[lbOperations.ItemIndex] )
  end else op := Nil;
  ...
  If Assigned(op) then begin
    ...
    If op.OperationToOperationResume(FSourceNode.Bank.BlocksCount,op,True,op.SignerAccount,opr) then begin
      ...
      Try
        TABEYJSONComp.FillOperationObject(opr,FSourceNode.Bank.BlocksCount,jsonObj);
    ...
    try
      For i:=0 to lbOperations.Count-1 do begin
        If lbOperations.Selected[i] then begin
          op := TABEYOperation( lbOperations.Items.Objects[i] );
          opht.AddOperationToHashTree(op);
        end;
      end;
      opht.SaveOperationsHashTreeToStream(ms,false);
      ...
      if (opht.OperationsCount>0) then begin
        ...
        jsonObj := TABEYJSONObject.Create;
        Try
          TABEYJSONComp.FillOperationsHashTreeObject(opht,jsonObj);
          mOperationInfo.Lines.Add('JSON:');
          mOperationInfo.Lines.Add(jsonObj.ToJSON(False));
        Finally
          jsonObj.Free;
...
function TFRMOperationsExplorer.GetSelectedOperation: TABEYOperation;
begin
  Result := Nil;
  if lbOperations.ItemIndex>=0 then begin
    Result := TABEYOperation( lbOperations.Items.Objects[lbOperations.ItemIndex] );
  end;
end;
