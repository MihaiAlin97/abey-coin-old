...
type
  ...
  TSearchThread = Class(TABEYThread)
  private
...
procedure TSearchThread.BCExecute;
  ...
  begin
    ...
    c := 0;
    maxC := FSearchValues.Vault.AccountsCount-1;
    ...
    while (c<=maxC) And (Not Terminated) And (Not FDoStopSearch) do begin
      account := FSearchValues.Vault.Account(c);
      isValid := True;
...
begin
  ...
  while (c<FVault.AccountsCount) do begin
    account := FVault.Account(c);
  ...
  Try
    ...
    sl.Add(Format('Current block:%d accounts:%d coins:%s',[FVault.BlocksCount,FVault.AccountsCount,TAccountComp.FormatMoney(FVault.TotalBalance)]));
    sl.Add('');
    sl.Add('** Basic stats **');
...
begin
  ...
  If (searchValues.inAccountKey.EC_OpenSSL_NID=0) AND (searchValues.inWalletKeys=Nil) And (searchValues.maxBal<0) And (searchValues.minBal<=0) And
    ...
    lblAccountsCount.Caption := IntToStr(FAccountsGrid.Node.Bank.Vault.AccountsCount);
