...
type
  ...
  TABEYMiningApp = class(TCustomApplication)
...
Const
  CT_MINER_VERSION = {$IFDEF PRODUCTION}'1.0'{$ELSE}{$IFDEF TESTNET}'1.0 ABEY_TESTNET'{$ELSE}ERROR{$ENDIF}{$ENDIF};
  ...
  CT_Line_Logs = 15;
  CT_MaxLogs = 10;
  CT_OpenCL_FileName = 'sha.cl';

{ TABEYMiningApp }

procedure TABEYMiningApp.ShowGPUDrivers;
Var i,j,n : Integer;
  dev : TDCLDevice;
begin
  n := 0;
...
procedure TABEYMiningApp.OnConnectionStateChanged(Sender: TObject);
...
procedure TABEYMiningApp.OnDeviceStateChanged(Sender: TObject);
Var i : Integer;
  s : String;
begin
...
procedure TABEYMiningApp.OnMinerValuesChanged(Sender: TObject);
begin
  If Sender is TCustomMinerDeviceThread then begin
...
procedure TABEYMiningApp.OnFoundNOnce(Sender: TCustomMinerDeviceThread; Timestamp, nOnce: Cardinal);
begin
...
procedure TABEYMiningApp.WriteLine(nline: Integer; txt: String);
Var i : Integer;
begin
  FLock.Acquire;
  try
...
procedure TABEYMiningApp.OnInThreadNewLog(logtype: TLogType; Time: TDateTime;
...
procedure TABEYMiningApp.DoRun;
var
  ErrorMsg: String;
  s : String;
  ...
  Begin
    ...
    try
      ...
      try
        ...
        WriteLine(1,'** ABEY miner ** Version: '+CT_MINER_VERSION);
...
constructor TABEYMiningApp.Create(TheOwner: TComponent);
Var FLog : TLog;
begin
  ...
  FLog.FileName:=ExtractFileDir(ExeName)+PathDelim+'ABEY_miner.log';
end;

destructor TABEYMiningApp.Destroy;
begin
  FreeAndNil(FDeviceThreads);
  inherited Destroy;
end;

procedure TABEYMiningApp.WriteHelp;
begin
  { add your help code here }
  writeln('ABEY Miner - Version: ',CT_MINER_VERSION);
  writeln('Usage: ', ExtractFileName(ExeName), ' -h -s S -p X -d Y -c N -n MYNAME');
  writeln('  -h for help');
  writeln('  -s S  (S is ABEY server:port where default value is localhost:',CT_JSONRPCMinerServer_Port,')');
  ...
  writeln('  (Needed only by some third party pools)');
  writeln('  -u USERNAME');
  writeln('  -x PASSWORD');
  writeln('');
...
var
  Application: TABEYMiningApp;
begin
  Application:=TABEYMiningApp.Create(nil);
  Application.Title:='ABEY Miner';
  Application.Run;
  Application.Free;
end.
