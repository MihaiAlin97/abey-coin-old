...
type
  TABEYGRandomNumberGenerator = class sealed(TRandomNumberGenerator,
...
begin
  ...
  case rngMode of
    ...
    TRandomNumberGeneratorMode.rngmPCG:
      begin
        result := TABEYGRandomNumberGenerator.Create();
        Exit;
      end
...
{ TABEYGRandomNumberGenerator }

constructor TABEYGRandomNumberGenerator.Create;
begin
  inherited Create();
end;

procedure TABEYGRandomNumberGenerator.GetBytes(const data: TCryptoLibByteArray);
var
  i: Int64;
begin
  ...
  while i > 0 do
  begin
    data[i - 1] := Byte(TABEYg.NextInt(System.Low(Int32), System.High(Int32)));
    System.Dec(i);
  end;

end;

procedure TABEYGRandomNumberGenerator.GetNonZeroBytes
  (const data: TCryptoLibByteArray);
var
  i: Int64;
  val: Byte;
begin
  ...
  begin
    repeat
      val := Byte(TABEYg.NextUInt32(System.Low(UInt32), System.High(UInt32)));
    until (not(val = 0));
