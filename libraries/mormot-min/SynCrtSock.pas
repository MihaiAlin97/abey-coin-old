...

{
  ...
  ***************************************
  ...
  - new TWinHTTP class, using WinHTTP API (faster than THtTABEYlientSocket):
  ...
  - fixed long-standing random bug occuring when a TCrtSocket connection is
    closed then reopened (e.g. when THtTABEYlientSocket.Request does its retrial);
...
var
  ...
  // - used e.g. by THttpRequest, TSQLHtTABEYlientRequest and TSQLHtTABEYlientGeneric
...
{$endif FPC}
  ...
  /// parent of THtTABEYlientSocket and THttpServerSocket classes
  ...
  THtTABEYlientSocket = class(THttpSocket)
  protected
  ...
  // - may be either THtTABEYlientSocket or THtTABEYlientWebSockets (from
  // SynBidirSock unit)
  THtTABEYlientSocketClass = class of THtTABEYlientSocket;
  ...
  // - allow easy propagation e.g. from a TSQLHtTABEYlient* wrapper class to
...
{$ifdef USEWININET}
  ...
  // - note: WinINet is MUCH slower than THtTABEYlientSocket or TWinHttp: do not
  ...
  // - has a common behavior as THtTABEYlientSocket() but seems to be faster
...
function MainHtTABEYlass: THttpRequestClass;
...
/// create a THtTABEYlientSocket, returning nil on error
// - useful to easily catch socket error exception ECrtSocket
function OpenHttp(const aServer, aPort: SockString; aTLS: boolean=false): THtTABEYlientSocket; overload;

/// create a THtTABEYlientSocket, returning nil on error
// - useful to easily catch socket error exception ECrtSocket
function OpenHttp(const aURI: SockString; aAddress: PSockString=nil): THtTABEYlientSocket; overload;

/// retrieve the content of a web page, using the HTTP/1.1 protocol and GET method
// - this method will use a low-level THtTABEYlientSock socket: if you want
...
/// retrieve the content of a web page, using the HTTP/1.1 protocol and GET method
// - this method will use a low-level THtTABEYlientSock socket for plain http URI,
...
// - this method will use a low-level THtTABEYlientSock socket and its GetAuth method
// - if AuthToken<>'', will add an header with 'Authorization: Bearer '+AuthToken
...
const
  /// the layout of TSMTABEYonnection.FromText method
...
type
  ...
  TSMTABEYonnection = {$ifdef UNICODE}record{$else}object{$endif}
  public
...
// SendEmailSubject() to encode it as Unicode, if needed
// - you can optionally set the encoding charset to be used for the Text body
function SendEmail(const Server: TSMTABEYonnection;
...
constructor THtTABEYlientSocket.Create(aTimeOut: cardinal);
begin
  if aTimeOut=0 then
...
function THtTABEYlientSocket.Delete(const url: SockString; KeepAlive: cardinal;
  const header: SockString): integer;
begin
  result := Request(url,'DELETE',KeepAlive,header,'','',false);
end;

function THtTABEYlientSocket.Get(const url: SockString; KeepAlive: cardinal=0; const header: SockString=''): integer;
begin
...
function THtTABEYlientSocket.GetAuth(const url, AuthToken: SockString; KeepAlive: cardinal=0): integer;
begin
  result := Get(url,KeepAlive,AuthorizationBearer(AuthToken));
end;

function THtTABEYlientSocket.Head(const url: SockString; KeepAlive: cardinal;
  const header: SockString): integer;
begin
  result := Request(url,'HEAD',KeepAlive,header,'','',false);
end;

function THtTABEYlientSocket.Post(const url, Data, DataType: SockString; KeepAlive: cardinal;
  const header: SockString): integer;
begin
  result := Request(url,'POST',KeepAlive,header,Data,DataType,false);
end;

function THtTABEYlientSocket.Put(const url, Data, DataType: SockString;
  KeepAlive: cardinal; const header: SockString): integer;
begin
  result := Request(url,'PUT',KeepAlive,header,Data,DataType,false);
end;

procedure THtTABEYlientSocket.RequestSendHeader(const url, method: SockString);
var aURL: SockString;
begin
  if fSock<=0 then
    exit;
...
function THtTABEYlientSocket.Request(const url, method: SockString;
...
function OpenHttp(const aServer, aPort: SockString; aTLS: boolean): THtTABEYlientSocket;
begin
  try
    result := THtTABEYlientSocket.Open(aServer,aPort,cslTCP,0,aTLS); // HTTP_DEFAULT_RECEIVETIMEOUT
  except
    on ECrtSocket do
      result := nil;
  end;
...
function OpenHttp(const aURI: SockString; aAddress: PSockString): THtTABEYlientSocket;
var URI: TURI;
begin
  result := nil;
...
function TSMTABEYonnection.FromText(const aText: SockString): boolean;
var u,h: SockString;
begin
...
function SendEmail(const Server: TSMTABEYonnection; const From, CSVDest, Subject,
...
{$endif}
  ...
  HTTP_DATA_CHUNK_INMEMORY = record
    DataChunkType: THtTABEYhunkType; // always hctFromMemory
    Reserved1: ULONG;
    pBuffer: pointer;
    BufferLength: ULONG;
    Reserved2: ULONG;
  ...
  HTTP_DATA_CHUNK_FILEHANDLE = record
    DataChunkType: THtTABEYhunkType; // always hctFromFileHandle
    ByteRange: HTTP_BYTE_RANGE;
    FileHandle: THandle;
  end;
  HTTP_DATA_CHUNK_FRAGMENTCACHE = record
    DataChunkType: THtTABEYhunkType; // always hctFromFragmentCache
...
type
  ...
  THttpAPI = packed record
    ...
    /// The HtTABEYreateHttpHandle function creates an HTTP request queue for the
...
const
  ...
  HttpNames: array[THttpAPIs] of PChar = (
    'HttpInitialize','HttpTerminate','HtTABEYreateHttpHandle',
    ...
    'HtTABEYancelHttpRequest',
    'HtTABEYreateServerSession', 'HtTABEYloseServerSession',
    'HtTABEYreateRequestQueue',
    'HttpSetServerSessionProperty', 'HttpQueryServerSessionProperty',
    'HtTABEYreateUrlGroup', 'HtTABEYloseUrlGroup',
...
const
  WinHttpNames: array[TWinHttpAPIs] of PChar = (
    'WinHttpOpen', 'WinHttpSetStatusCallback', 'WinHtTABEYonnect',
    'WinHttpOpenRequest', 'WinHtTABEYloseHandle', 'WinHttpAddRequestHeaders',
...
type
  ...
  TCurlResult = (
    ...
    crFTPWeird227Format, crFTABEYantGetHost, crFTABEYantReconnect, crFTABEYouldntSetBINARY,
    crPartialFile, crFTABEYouldntRetrFile, crFTPWriteError, crFTPQuoteError,
    crHTTPReturnedError, crWriteError, crMalFormatUser, crFTABEYouldntStorFile,
    crReadError, crOutOfMemory, crOperationTimeouted,
    crFTABEYouldntSetASCII, crFTPPortFailed, crFTABEYouldntUseREST, crFTABEYouldntGetSize,
...
var
  _MainHtTABEYlass: THttpRequestClass;

function MainHtTABEYlass: THttpRequestClass;
begin
  if _MainHtTABEYlass = nil then begin
    {$ifdef USEWININET}
    _MainHtTABEYlass := TWinHTTP;
    {$else}
    {$ifdef USELIBCURL}
    _MainHtTABEYlass := TCurlHTTP
    {$else}
    raise ECrtSocket.Create('No THttpRequest class known!');
    {$endif}
    {$endif}
  end;
  result := _MainHtTABEYlass;
end;
