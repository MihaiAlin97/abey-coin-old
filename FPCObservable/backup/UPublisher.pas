...
uses
  ...
  Classes,SysUtils,blcksock,synsock,lazlogger,contnrs,typinfo,syncobjs,UJSONCommunication,fgl,ULog;
...
Type
    ...
    TNewSubscriptionEvent = procedure(Event:BlockchainEvent) of Object;
 ...
 // this thread makes changes on every element of the pattern : Listener thread, communication threads(pool), Publisher -> this should block other threads
    ...
    TPublisher = class
      ...
      public
        ...
        procedure HandleNewSubscription(Event:BLockchainEvent);
...
uses UBlockchain;
  ...
  begin
    ...
    for I := 1 to FTaskManager.FWorkerPool.Count do begin
  ...
  procedure TPublisher.HandleNewSubscription(Event:BLockchainEvent);

  begin
    TLog.NewLog(lterror,ClassName,'Arrived to HandleSubscription: ');

    SetLength(EventList,Length(EventList) + 1 );
    EventList[Length(EventList) - 1 ] := Event;
  ...
  begin
    ...
    Done := False;
    while (not Terminated and Done = False) do
      begin
        DebugLn(NewStatus);
              ...
              if canread(2500) then begin
                ...
                if lastError=0 then begin
                  ...
                  if Message <> '' then begin
                    ...
                    fEvent := JSONHelper.GetEventFromSubscription(Message);
