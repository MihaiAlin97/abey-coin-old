...
uses
  ...
  UThread, UTime, SyncObjs, UCrypto, ULog, UConst;
    ...
    If (Paused) then begin
      ...
      //  AuxStats := CT_TMinerStats_NULL;
        If FReadyToGPU then begin
          ...
          if PoolMinerThread.GlobalMinerValuesForWork.block <= CT_Milestone2TimestampBlockNumber then begin
             Timestamp := CT_TimestampGenesis + (PoolMinerThread.GlobalMinerValuesForWork.block - 1) * 120 + 110 + Random(20);
          end;
  ...
  try
    If FNeedNewDevice then begin
      ...
      If Assigned(FDCLDevice) And (FProgramFileName<>'') And (FileExists(FProgramFileName)) then begin
        ...
        FKernelArg2 := 0; // Save nOnce=0 (not valid)
        FKernelOutputBuffer := FDCLDevice.CreateBuffer(4,@FKernelArg2,[mfReadWrite, mfCopyHostPtr {mfCopyHostPtr mfUseHostPtr}]);
        FDCLKernel := FDCLProgram.CreateKernel('abey_miner');
      end;
    end;
    UpdateBuffers;
  finally
...
begin
  ...
  try
    ...
    ABEYPrepareLastChunk(digest,stateforlastchunk,bufferForLastChunk);
