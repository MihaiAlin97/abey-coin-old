...
uses
  ...
  IniFiles, UBaseTypes,
  UThread, URPC, UPoolMining, UAccounts, dbstorage;
...
Type
  ...
  TABEYDaemonThread = Class(TABEYThread)
  private
    FIniFile : TIniFile;
  ...
  TABEYService = Class(TCustomDaemon)
  Private
    FThread : TABEYDaemonThread;
  ...
  TABEYDaemonMapper = Class(TCustomDaemonMapper)
  private
    FLog : TLog;
    procedure OnABEYInThreadLog(logtype : TLogType; Time : TDateTime; AThreadID : TThreadID; Const sender, logtext : String);
  protected
...
procedure TABEYDaemonThread.OnNetDataReceivedHelloMessage(Sender: TObject);
...
procedure TABEYDaemonThread.BCExecute;
var
  FNode : TNode;
  ...
  Begin
    ...
    If FIniFile.ReadBool(CT_INI_SECTION_GLOBAL,CT_INI_IDENT_RPC_SAVELOGS,true) then begin
      ...
      FRPC.LogFileName:= TFolderHelper.GetABEYDataFolder+PathDelim+'ABEY_RPC.log';
  ...
  Begin
    ...
    if (i>0) then begin
      ...
      if (SameText(s,'TIME')) then begin
        s := FormatDateTime('yyyy-mm-dd hh:nn',CT_TimestampGenesis);
        PayloadFix := '----------------';
        s := PayloadFix;
...
begin
  FMInerServer := Nil;
  TLog.NewLog(ltinfo,Classname,'START ABEY Server');
  try
    try
      ...
      Try
        ...
        TNetData.NetData.MinFutureBlocksToDownloadNewVault := FIniFile.ReadInteger(CT_INI_SECTION_GLOBAL,CT_INI_IDENT_MINPENDINGBLOCKSTODOWNLOADCHECKPOINT,0);
        ...
        // Reading database
        FNode.InitVaultAndOperations(MaxBlockToRead);
    ...
    TLog.NewLog(ltinfo,Classname,'EXIT ABEY Server');
  end;
end;

constructor TABEYDaemonThread.Create;
begin
  ...
  If FIniFile.ReadBool(CT_INI_SECTION_GLOBAL,CT_INI_IDENT_SAVELOGS,true) then begin
    ...
    _FLog.FileName:=TFolderHelper.GetABEYDataFolder+PathDelim+'ABEY_Daemon_'+FormatDateTime('yyyymmddhhnn',Now)+'.log';
...
destructor TABEYDaemonThread.Destroy;
begin
  FreeAndNil(FIniFile);
  inherited Destroy;
end;
...
procedure TABEYService.ThreadStopped(Sender: TObject);
begin
  FreeAndNil(FThread);
end;

function TABEYService.Start: Boolean;
begin
  Result:=inherited Start;
  TLog.NewLog(ltinfo,ClassName,'Daemon Start '+BoolToStr(Result));
  FThread:=TABEYDaemonThread.Create;
...
function TABEYService.Stop: Boolean;
begin
...
function TABEYService.Pause: Boolean;
begin
...
function TABEYService.Continue: Boolean;
begin
...
function TABEYService.Execute: Boolean;
begin
  Result:=inherited Execute;
  TLog.NewLog(ltinfo,ClassName,'Daemon execute: '+BoolToStr(Result));
end;

function TABEYService.ShutDown: Boolean;
begin
...
function TABEYService.Install: Boolean;
begin
  Result:=inherited Install;
  TLog.NewLog(ltinfo,ClassName,'Daemon Install: '+BoolToStr(Result));
end;

function TABEYService.UnInstall: boolean;
begin
...
procedure TABEYDaemonMapper.OnABEYInThreadLog(logtype: TLogType;
  Time: TDateTime; AThreadID: TThreadID; const sender, logtext: String);
Var s : AnsiString;
begin
//  If Not SameText(sender,TABEYDaemonThread.ClassName) then exit;
...
procedure TABEYDaemonMapper.DoOnCreate;
Var D : TDaemonDef;
begin
  inherited DoOnCreate;
  WriteLn('');
  WriteLn(formatDateTime('dd/mm/yyyy hh:nn:ss.zzz',now)+' Starting ABEY service');
  FLog := TLog.Create(Nil);
  FLog.OnInThreadNewLog:=@OnABEYInThreadLog;
  _FLog := FLog;
  D:=DaemonDefs.Add as TDaemonDef;
  D.DisplayName:='ABEY Service';
  D.Name:='ABEYService';
  D.DaemonClassName:='TABEYService';
  D.WinBindings.ServiceType:=stWin32;
end;

procedure TABEYDaemonMapper.DoOnDestroy;
begin
