...
Const
  ...
  CT_NetOp_GetVault           = $0021; // V2 Protocol: Allows to send/receive Vault in chunk parts
  ...
  CT_NetError_VaultNotFound = $0020;
  ...
  CT_MAX_VaultCHUNK_BLOCKS = 30000;
...
Type
  ...
  private
    ...
    FNetData : TNetData;
    FCritical : TABEYCriticalSection;
  ...
  TNetDataNotifyEventsThread = Class(TABEYThread)
  private
    FNetData: TNetData;
  ...
  TNetClientsDestroyThread = Class(TABEYThread)
  private
    FNetData : TNetData;
  ...
  TThreadCheckConnections = Class(TABEYThread)
  private
    FNetData : TNetData;
  ...
  private
    FTimesList : TABEYThreadList<Pointer>;
    FTimeOffset : Integer;
  ...
  TGetNewBlockchainFromClientDownloadNewVault = procedure (netData : TNetData; clientConnection : TNetConnection; my_blocks_count, client_blocks_count : Integer; var download_new_Vault : Boolean) of object;
  ...
  private
    ...
    FNetConnections : TABEYThreadList<TNetConnection>;
    ...
    FRegisteredRequests : TABEYThreadList<Pointer>;
    FIsDiscoveringServers : Boolean;
    FLockGettingNewBlockChainFromClient : TABEYCriticalSection;
    ...
    FIpInfos : TIpInfos;
    FMinFutureBlocksToDownloadNewVault: Integer;
    FOnGetNewBlockchainFromClientDownloadNewVault: TGetNewBlockchainFromClientDownloadNewVault;
    ...
    procedure SetMinFutureBlocksToDownloadNewVault(const Value: Integer);
  protected
  ...
  public
    ...
    procedure OnReadingNewVaultProgressNotify(sender : TObject; const mesage : String; curPos, totalCount : Int64);
    ...
    Property NetConnections : TABEYThreadList<TNetConnection> read FNetConnections;
    ...
    property OnGetNewBlockchainFromClientDownloadNewVault : TGetNewBlockchainFromClientDownloadNewVault read FOnGetNewBlockchainFromClientDownloadNewVault write FOnGetNewBlockchainFromClientDownloadNewVault;
    ...
    Property MinFutureBlocksToDownloadNewVault : Integer read FMinFutureBlocksToDownloadNewVault write SetMinFutureBlocksToDownloadNewVault;
  End;
  ...
  private
    ...
    FBufferLock : TABEYCriticalSection;
    ...
    Procedure DoProcess_GetVault_Request(HeaderData : TNetHeaderData; DataBuffer: TStream);
  ...
  public
    ...
    Function Send_NewBlockFound(Const NewBlock : TABEYOperationsComp) : Boolean;
  ...
  TNetClient = Class;
  TNetClientThread = Class(TABEYThread)
  private
  ...
  TThreadDiscoverConnection = Class(TABEYThread)
  ...
  TThreadGetNewBlockChainFromClient = Class(TABEYThread)
  protected
    procedure BCExecute; override;
  public
    Constructor Create;
...
begin
  FNetData := ANetData;
  FCritical := TABEYCriticalSection.Create(Classname);
...
function TNetData.Bank: TABEYBank;
begin
...
begin
  ...
  FOnGetNewBlockchainFromClientDownloadNewVault := Nil;
  FIsDiscoveringServers := false;
  FRegisteredRequests := TABEYThreadList<Pointer>.Create('TNetData_RegisteredRequests');
  FNodeServersAddresses := TOrderedServerAddressListTS.Create(Self);
  FLastRequestId := 0;
  FNetConnections := TABEYThreadList<TNetConnection>.Create('TNetData_NetConnections');
  FLockGettingNewBlockChainFromClient := TABEYCriticalSection.Create('LockGettingNewBlockChainFromClient');
  ...
  // download blocks, instead will download directly new Vault state
  MinFutureBlocksToDownloadNewVault := (86400 DIV CT_NewLineSecondsAvg) * {$IFDEF PRODUCTION}7{$ELSE}1{$ENDIF}; // Only 1 day for TESTNET, 7 for PRODUCTION
  //
...
begin
  ...
  Repeat
    tdc := TThreadDiscoverConnection( TABEYThreadClass.GetThreadByClass(TThreadDiscoverConnection,nil) );
...
begin
  ...
  if TABEYThread.ThreadClassFound(TThreadDiscoverConnection,nil)>=0 then begin
...
begin
  NotifyNodeServersUpdated;
  if TABEYThread.ThreadClassFound(TThreadDiscoverConnection,Nil)>=0 then exit;
  FIsDiscoveringServers := false;
  ...
  if TABEYThread.ThreadClassFound(TThreadGetNewBlockChainFromClient,nil)>=0 then exit;
...
Const CT_LogSender = 'GetNewBlockChainFromClient';
  ...
  function Do_GetOperationsBlock(AssignToBank : TABEYBank; block_start,block_end, MaxWaitMilliseconds : Cardinal; OnlyOperationBlock : Boolean; BlocksList : TList<TABEYOperationsComp>) : Boolean;
  Var SendData,ReceiveData : TMemoryStream;
    headerdata : TNetHeaderData;
    op : TABEYOperationsComp;
    ...
    try
      ...
      if Connection.DoSendAndWaitForResponse(noperation,request_id,SendData,ReceiveData,MaxWaitMilliseconds,headerdata) then begin
        ...
        while (i<opcount) do begin
          // decode data
          op := TABEYOperationsComp.Create(AssignToBank);
  ...
  Var BlocksList : TList<TABEYOperationsComp>;
    i : Integer;
  begin
    OperationBlock := CT_OperationBlock_NUL;
    BlocksList := TList<TABEYOperationsComp>.Create;
    try
      ...
      if (Result) And (BlocksList.Count=1) then begin
        OperationBlock := TABEYOperationsComp(BlocksList[0]).OperationBlock;
        If OperationBlock.block<>block then Result := False;
      end else begin
        Result := False;
      end;
    finally
      for i := 0 to BlocksList.Count - 1 do TABEYOperationsComp(BlocksList[i]).Free;
      BlocksList.Free;
    end;
  end;
  ...
  var i : Integer;
    ...
    BlocksList : TList<TABEYOperationsComp>;
    errors : String;
  Begin
    Result := false;
    OperationBlock := CT_OperationBlock_NUL;
    repeat
      ...
      try
        ...
        for i := 0 to BlocksList.Count - 1 do begin
          auxBlock := TABEYOperationsComp(BlocksList[i]).OperationBlock;
          ...
          If Not TABEYVault.IsValidOperationBlock(auxBlock,errors) then begin
            Connection.DisconnectInvalidClient(false,'Received invalid operation block searching '+TABEYOperationsComp.OperationBlockToText(auxBlock)+' errors: '+errors);
            Exit;
          end;

          ant_nblock := auxBlock.block;
          //
          sbBlock := TNode.Node.Bank.Vault.Block(auxBlock.block).blockchainInfo;
          if TABEYOperationsComp.EqualsOperationBlock(sbBlock,auxBlock) then begin
        ...
        for i := 0 to BlocksList.Count - 1 do begin
          TABEYOperationsComp(BlocksList[i]).Free;
        end;
        BlocksList.Free;
      end;
  ...
  Var BlocksList : TList<TABEYOperationsComp>;
    i : Integer;
    OpComp,OpExecute : TABEYOperationsComp;
  ...
  Begin
    ...
    try
      ...
      if start_block>=0 then begin
        If (TNode.Node.Bank.Vault.HasSnapshotForBlock(start_block-1)) then begin
          // Restore from a Snapshot (New on V3) instead of restore reading from File
          Bank.Vault.SetToPrevious(TNode.Node.Bank.Vault,start_block-1);
          Bank.UpdateValuesFromVault;
      ...
      repeat
        ...
        try
          ...
          while (i<BlocksList.Count) And (Not finished) do begin
            OpComp := TABEYOperationsComp(BlocksList[i]);
            ms := TMemoryStream.Create;
            OpExecute := TABEYOperationsComp.Create(Bank);
            try
              ...
              if Bank.AddNewBlockChainBlock(OpExecute,TNetData.NetData.NetworkAdjustedTime.GetMaxAllowedTimestampForNewBlock,newBlock,errors) then begin
                ...
                TLog.NewLog(lterror,CT_LogSender,'Error creating new bank with client Operations. Block:'+TABEYOperationsComp.OperationBlockToText(OpExecute.OperationBlock)+' Error:'+errors);
                // Add to blacklist !
                Connection.DisconnectInvalidClient(false,'Invalid BlockChain on Block '+TABEYOperationsComp.OperationBlockToText(OpExecute.OperationBlock)+' with errors:'+errors);
                finished := true;
                IsAScam := true;
                break;
              end;
      ...
      until (Bank.BlocksCount=Connection.FRemoteOperationBlock.block+1) Or (finished)
        ...
        Or (Bank.Vault.WorkSum > (TNode.Node.Bank.Vault.WorkSum + $FFFFFFFF) );
      ...
      if Bank.Vault.WorkSum > TNode.Node.Bank.Vault.WorkSum then begin
        oldBlockchainOperations := TOperationsHashTree.Create;
        try
          TNode.Node.DisableNewBlocks;
          Try
            // I'm an orphan blockchain...
            TLog.NewLog(ltinfo,CT_LogSender,'New valid blockchain found. My block count='+inttostr(TNode.Node.Bank.BlocksCount)+' work: '+IntToStr(TNode.Node.Bank.Vault.WorkSum)+
              ' found count='+inttostr(Bank.BlocksCount)+' work: '+IntToStr(Bank.Vault.WorkSum)+' starting at block '+inttostr(start_block));
            if TNode.Node.Bank.BlocksCount>0 then begin
              OpExecute := TABEYOperationsComp.Create(Nil);
              try
            ...
            If IsUsingSnapshot then begin
              TLog.NewLog(ltInfo,CT_LogSender,'Commiting new chain to Vault');
              Bank.Vault.CommitToPrevious;
              TNode.Node.Bank.UpdateValuesFromVault; // BUG 2018-10-14 -> Must update TNode.Node.Bank instead of Bank, because FLastBlockCache must upgrade
              {$IFDEF Check_Vault_Names_Consistency}
              If Not Check_Vault_Names_Consistency(Bank.Vault,'Commited',errors) then begin
                TLog.NewLog(lterror,CT_LogSender,'Fatal Vault consistency error getting bank at block '+IntTosTr(start_block)+' : '+errors);
                Sleep(1000);
                halt(0);
              end;
              {$ENDIF}
            end else begin
              TLog.NewLog(ltInfo,CT_LogSender,'Restoring modified Vault from Disk');
        ...
        if (Not IsAScam) And (Connection.FRemoteAccumulatedWork > TNode.Node.Bank.Vault.WorkSum) then begin
          // Possible scammer!
          Connection.DisconnectInvalidClient(false,Format('Possible scammer! Says blocks:%d Work:%d - Obtained blocks:%d work:%d',
            [Connection.FRemoteOperationBlock.block+1,Connection.FRemoteAccumulatedWork,
             Bank.BlocksCount,Bank.Vault.WorkSum]));
        end;
      end;
    finally
      Bank.Free;
  ...
  Function DownloadVaultChunk(Vault_blockscount : Cardinal; Const sbh : TRawBytes; from_block, to_block : Cardinal; receivedDataUnzipped : TStream;
    var VaultHeader : TABEYVaultHeader; var errors : String) : Boolean;
  ...
  Begin
    ...
    receiveData := TMemoryStream.Create;
    try
      sendData.Write(Vault_blockscount,SizeOf(Vault_blockscount)); // 4 bytes for blockcount
      TStreamOp.WriteAnsiString(SendData,sbh);
      sendData.Write(from_block,SizeOf(from_block));
      c := to_block;
      if (c>=Vault_blockscount) then c := Vault_blockscount-1;
      sendData.Write(c,SizeOf(c));
      if (from_block>c) or (c>=Vault_blockscount) then begin
      ...
      if Connection.NetProtocolVersion.protocol_version<9 then begin
        ...
        TLog.NewLog(ltDebug,CT_LogSender,Format('Sleep %d miliseconds prior to Call to GetVault from blocks %d to %d of %d',[LRandomMilis,from_block,c,Vault_blockscount]));
        Sleep(LRandomMilis);
      end;
      TLog.NewLog(ltDebug,CT_LogSender,Format('Call to GetVault from blocks %d to %d of %d',[from_block,c,Vault_blockscount]));
      request_id := TNetData.NetData.NewRequestId;
      if Connection.DoSendAndWaitForResponse(CT_NetOp_GetVault,request_id,sendData,receiveData,30000,headerdata) then begin
        if HeaderData.is_error then exit;
        receivedDataUnzipped.Size:=0;
        If Not TABEYChunk.LoadVaultFromChunk(receiveData,receivedDataUnzipped,VaultHeader,errors) then begin
          Connection.DisconnectInvalidClient(false,'Invalid received chunk: '+errors);
          exit;
        end;
        If (Not (TBaseType.Equals(VaultHeader.VaultHash,sbh))) or (VaultHeader.startBlock<>from_block) or (VaultHeader.endBlock<>c) or
          (VaultHeader.blocksCount<>Vault_blockscount) or (VaultHeader.protocol<CT_PROTOCOL_2) or
          (VaultHeader.protocol>CT_BlockChain_Protocol_Available) then begin
          errors := Format('Invalid received chunk based on call: Blockscount:%d %d - from:%d %d to %d %d - VaultHash:%s %s',
              [VaultHeader.blocksCount,Vault_blockscount,VaultHeader.startBlock,from_block,VaultHeader.endBlock,c,
               VaultHeader.VaultHash.ToHexaString,sbh.ToHexaString]);
          Connection.DisconnectInvalidClient(false,'Invalid received chunk: '+errors);
          exit;
        end;
        Result := True;
      end else errors := 'No response on DownloadVaultChunk';
    finally
      receiveData.Free;
      SendData.Free;
    end;
  ...
  Type TVaultChunkData = Record
    VaultHeader : TABEYVaultHeader;
    chunkStream : TStream;
  end;

  Function DownloadVaultStream(VaultStream : TStream; var Vault_last_operation_block : TOperationBlock) : Boolean;
  var _blockcount, request_id : Cardinal;
    chunks : Array of TVaultChunkData;
  ...
  Begin
    ...
    // Will try to download penultimate saved Vault
    _blockcount := ((Connection.FRemoteOperationBlock.block DIV CT_BankToDiskEveryNBlocks)-1) * CT_BankToDiskEveryNBlocks;
    If not Do_GetOperationBlock(_blockcount,5000,Vault_last_operation_block) then begin
      Connection.DisconnectInvalidClient(false,Format('Cannot obtain operation block %d for downloading Vault',[_blockcount]));
      exit;
    end;
    // New Build 2.1.7 - Check valid operationblock
    If Not TABEYVault.IsValidOperationBlock(Vault_last_operation_block,errors) then begin
      Connection.DisconnectInvalidClient(false,'Invalid operation block at DownloadVault '+TABEYOperationsComp.OperationBlockToText(Vault_last_operation_block)+' errors: '+errors);
      Exit;
    end;
    SetLength(chunks,0);
    try
      // Will obtain chunks of 10000 blocks each -> Note: Maximum is CT_MAX_VaultCHUNK_BLOCKS
      for i:=0 to ((_blockcount-1) DIV 10000) do begin // Bug v3.0.1 and minors
        FNewBlockChainFromClientStatus := Format('Downloading vault with %d blocks (%d of %d) from %s',
          [_blockcount,i+1,((_blockcount-1) DIV 10000)+1,Connection.ClientRemoteAddr]);
        receiveChunk := TMemoryStream.Create;
        if (Not DownloadVaultChunk(_blockcount,Vault_last_operation_block.initial_safe_box_hash,(i*10000),((i+1)*10000)-1,receiveChunk,VaultHeader,errors)) then begin
          receiveChunk.Free;
          TLog.NewLog(ltError,CT_LogSender,errors);
          Exit;
        end;
        SetLength(chunks,length(chunks)+1);
        chunks[High(chunks)].VaultHeader := VaultHeader;
      ...
      try
        if (length(chunks)=1) then begin
          VaultStream.CopyFrom(chunks[0].chunkStream,0);
        end else begin
        ...
        for i:=1 to high(chunks) do begin
          ...
          If Not TABEYVault.ConcatVaultStream(chunk1,chunks[i].chunkStream,VaultStream,errors) then begin
            TLog.NewLog(ltError,CT_LogSender,errors);
            exit;
          end;
          chunk1.Size := 0;
          chunk1.CopyFrom(VaultStream,0);
        end;
      finally
        chunk1.Free;
      end;
  ...
  Function DownloadVault(IsMyBlockchainValid : Boolean) : Boolean;
  ...
  Begin
    ...
    receiveData := TMemoryStream.Create;
    try
      if Not DownloadVaultStream(receiveData,op) then Exit;
      // Now receiveData is the ALL Vault
      TNode.Node.DisableNewBlocks;
      try
          FNewBlockChainFromClientStatus := Format('Received new Vault with %d blocks from %s',[op.block+1,Connection.ClientRemoteAddr]);
          receiveData.Position:=0;
          If TNode.Node.Bank.LoadBankFromStream(receiveData,True,op.initial_safe_box_hash,TNode.Node.Bank.Vault,OnReadingNewVaultProgressNotify,errors) then begin
            TLog.NewLog(ltInfo,ClassName,'Received new Vault!');
  ...
  var VaultStream : TMemoryStream;
    newTmpBank : TABEYBank;
    Vault_last_operation_block : TOperationBlock;
    newBlock : TBlockAccount;
    opComp : TABEYOperationsComp;
    errors : String;
    blocksList : TList<TABEYOperationsComp>;
    i : Integer;
    rid : Cardinal;
    download_new_Vault : Boolean;
  begin
    download_new_Vault := (FMinFutureBlocksToDownloadNewVault>0) And ((TNode.Node.Bank.BlocksCount + FMinFutureBlocksToDownloadNewVault) <= Connection.RemoteOperationBlock.block);
    if Assigned(OnGetNewBlockchainFromClientDownloadNewVault) then begin
      // Note: Will call to an event inside a thread, not main thread, be careful
      OnGetNewBlockchainFromClientDownloadNewVault(Self,Connection,TNode.Node.Bank.BlocksCount,Connection.RemoteOperationBlock.block,download_new_Vault);
    end;
    if (download_new_Vault) then begin
      TLog.NewLog(ltinfo,ClassName,Format('Will download new Vault. My blocks:%d Remote blocks:%d Equal Block:%d (MaxFutureBlocksToDownloadNewVault:%d)',[TNode.Node.Bank.BlocksCount,Connection.RemoteOperationBlock.block+1,start_block-1,MinFutureBlocksToDownloadNewVault]));
      // Will try to download Vault
      VaultStream := TMemoryStream.Create;
      Try
        if Not DownloadVaultStream(VaultStream,Vault_last_operation_block) then Exit;
        VaultStream.Position := 0;
        newTmpBank := TABEYBank.Create(Nil);
        try
          ...
          if newTmpBank.LoadBankFromStream(VaultStream,True,Vault_last_operation_block.initial_safe_box_hash,TNode.Node.Bank.Vault,OnReadingNewVaultProgressNotify,errors) then begin
            TNode.Node.DisableNewBlocks;
            try
              TLog.NewLog(ltInfo,ClassName,'Received new Vault!');
              ...
              blocksList := TList<TABEYOperationsComp>.Create;
              try
                if Not Do_GetOperationsBlock(newTmpBank,Vault_last_operation_block.block,Vault_last_operation_block.block+10,20000,False,blocksList) then begin
                  TLog.NewLog(ltError,ClassName,Format('Cannot receive at least 1 new block:%d',[Vault_last_operation_block.block]));
                  Exit;
                end;
                for i:=0 to blocksList.Count-1 do begin
                  opComp := TABEYOperationsComp( blocksList[i] );
                  if Not newTmpBank.AddNewBlockChainBlock(opComp,TNetData.NetData.NetworkAdjustedTime.GetMaxAllowedTimestampForNewBlock,newBlock,errors) then begin
                    TLog.NewLog(lterror,CT_LogSender,'Error adding new block with client Operations. Block:'+TABEYOperationsComp.OperationBlockToText(opComp.OperationBlock)+' Error:'+errors);
                    // Add to blacklist !
                    Connection.DisconnectInvalidClient(false,'Invalid BlockChain on Block '+TABEYOperationsComp.OperationBlockToText(opComp.OperationBlock)+' with errors:'+errors);
                    Exit;
                  end;
                end;
              finally
              ...
              // We are ready to upgrade with newest Vault
              // Delete blocks since start_block at current TNode
              TNode.Node.Bank.Storage.MoveBlockChainBlocks(start_block,IntToStr(start_block)+'_'+FormatDateTime('yyyymmddhhnnss',DateTime2UnivDateTime(now)),Nil);
              TNode.Node.Bank.Storage.DeleteBlockChainBlocks(start_block);

              newTmpBank.Storage.MoveBlockChainBlocks(Vault_last_operation_block.block,'',TNode.Node.Bank.Storage);
              TNode.Node.Bank.DiskRestoreFromOperations(CT_MaxBlock);
            Finally
              TNode.Node.EnableNewBlocks;
            End;
...
begin
  ...
  Try
    TLog.NewLog(ltdebug,CT_LogSender,'Receiving: '+why);
    ...
    if TNode.Node.Bank.BlocksCount=0 then begin
      TLog.NewLog(ltdebug,CT_LogSender,'There are no blocks');
    ...
    TLog.NewLog(ltdebug,CT_LogSender,'Starting GetNewBlockChainFromClient at client:'+Connection.ClientRemoteAddr+
      ' with OperationBlock:'+TABEYOperationsComp.OperationBlockToText(Connection.FRemoteOperationBlock)+' (My block: '+TABEYOperationsComp.OperationBlockToText(TNode.Node.Bank.LastOperationBlock)+')');
    ...
    // New Build 2.1.7 - Check valid operationblock
    If Not TABEYVault.IsValidOperationBlock(client_op,errors) then begin
      Connection.DisconnectInvalidClient(false,'Received invalid operation block '+TABEYOperationsComp.OperationBlockToText(client_op)+' errors: '+errors);
      Exit;
    end;

    if (NOT TABEYOperationsComp.EqualsOperationBlock(my_op,client_op)) then begin
      if (my_op.protocol_version > client_op.protocol_version) then begin // Version 4.0.2 protection against going back to previous protocol with highest blockchain
        TABEYOperationsComp.OperationBlockToText(my_op);
        TLog.NewLog(lterror,CT_LogSender,Format('Detected an orphan highest blockchain in an old protocol. Detected: %s - My data: %s',[TABEYOperationsComp.OperationBlockToText(client_op),TABEYOperationsComp.OperationBlockToText(my_op)]));
        Connection.DisconnectInvalidClient(false,'Detected an orphan highest blockchain in an old protocol');
        Exit;
      end;
      TLog.NewLog(ltinfo,CT_LogSender,'My blockchain is not equal... received: '+TABEYOperationsComp.OperationBlockToText(client_op)+' My: '+TABEYOperationsComp.OperationBlockToText(my_op));
      if Not FindLastSameBlockByOperationsBlock(0,client_op.block,client_op) then begin
        ...
        if (TNode.Node.Bank.Storage.FirstBlock<client_op.block) then begin
          TLog.NewLog(ltinfo,CT_LogSender,'Found base new block: '+TABEYOperationsComp.OperationBlockToText(client_op));
          DownloadNewBlockchain(client_op.block+1,False);
        end else begin
          TLog.NewLog(ltinfo,CT_LogSender,'Found base new block: '+TABEYOperationsComp.OperationBlockToText(client_op)+' lower than saved:'+IntToStr(TNode.Node.Bank.Storage.FirstBlock));
          DownloadVault(False);
        end;
      end;
    end else begin
...
procedure TNetData.SetMinFutureBlocksToDownloadNewVault(const Value: Integer);
begin
  // Will allow a minimum of 200 future blocks fo enable download a new Vault
  if (Value<=200) then FMinFutureBlocksToDownloadNewVault := 0
  else FMinFutureBlocksToDownloadNewVault := Value;
end;
...
procedure TNetData.OnReadingNewVaultProgressNotify(sender: TObject; const mesage: String; curPos, totalCount: Int64);
Var pct : String;
begin
  if (totalCount>0) then pct := FormatFloat('0.00',curPos*100/totalCount)+'%' else pct := '';

  FNewBlockChainFromClientStatus := Format('Checking new Vault: %s %s',[mesage,pct]);
end;
...
begin
  case operation of
    ...
    CT_NetOp_GetVault : Result := 'GET_Vault';
...
begin
  ...
  Try
    ...
    TABEYThread.ProtectEnterCriticalSection(Self,FNetLock);
    Try
...
begin
  ...
  FNetLock := TABEYCriticalSection.Create('TNetConnection_NetLock');
  FLastHelloTS := 0;
  FLastDataReceivedTS := 0;
  FLastDataSendedTS := 0;
  FRandomWaitSecondsSendHello := (CT_RandomHelloRefactoring DIV 3) + Random(CT_RandomHelloRefactoring DIV 2);
  FTcpIpClient := Nil;
  ...
  TNetData.NetData.NotifyNetConnectionUpdated;
  FBufferLock := TABEYCriticalSection.Create('TNetConnection_BufferLock');
...
begin
  ...
  try
    ...
    for i := 1 to c do begin
      ...
      opclass := TABEYOperationsComp.GetOperationClassByOpType(optype);
...
procedure TNetConnection.DoProcess_GetBlockchainOperations_Request(HeaderData: TNetHeaderData; DataBuffer: TStream);
  ...
  function GetBlock(bufferOperationsBlock : TList<TABEYOperationsComp>; nBlock : Integer) : TABEYOperationsComp;
  var i : Integer;
  begin
    // Search at buffer:
    i := 0; Result := Nil;
    while (i<bufferOperationsBlock.Count) And (TABEYOperationsComp( bufferOperationsBlock[i] ).OperationBlock.block <> nBlock) do inc(i);
    if (i>=bufferOperationsBlock.Count) then begin
      // Not found !
      Result := TABEYOperationsComp.Create(Nil);
    ...
    end else Result := TABEYOperationsComp( bufferOperationsBlock[i] );
  end;
Var input_operations_count, cBlock, cBlockOpIndex, c : Cardinal;
  block_op_ref : UInt64;
  i : Integer;
  bufferOperationsBlock : TList<TABEYOperationsComp>;
  opc : TABEYOperationsComp;
...
begin
  ...
  try
    ...
    bufferOperationsBlock := TList<TABEYOperationsComp>.Create;
    ...
    Finally
      ...
      for i := 0 to bufferOperationsBlock.Count-1 do begin
        TABEYOperationsComp(bufferOperationsBlock[i]).Free;
      end;
...
begin
  ...
  try
    if HeaderData.header_type<>ntp_request then begin
     ...
     db := TMemoryStream.Create;
     try
       op := TABEYOperationsComp.Create(TNetData.NetData.bank);
       try
         c := b_end - b_start + 1;
...
procedure TNetConnection.DoProcess_GetBlocks_Response(HeaderData: TNetHeaderData; DataBuffer: TStream);
  var LTmpOp : TABEYOperationsComp;
  ...
  try
    ...
    Try
      ...
      for i := 0 to LOpCount-1 do begin
        LTmpOp := TABEYOperationsComp.Create(nil);
        try
      ...
      LVaultTransaction := TABEYVaultTransaction.Create(TNode.Node.Bank.Vault);
      try
        TABEYOperationsSignatureValidator.MultiThreadPreValidateSignatures(LVaultTransaction,LBlocks,Nil);
      finally
        ...
        if (LBlocks[i].OperationBlock.block=TNode.Node.Bank.BlocksCount) then begin
          if (TNode.Node.Bank.AddNewBlockChainBlock(LBlocks[i],TNetData.NetData.NetworkAdjustedTime.GetMaxAllowedTimestampForNewBlock, newBlockAccount,errors)) then begin
            ...
            TLog.NewLog(ltinfo,Classname,'Distinct operation block found! My:'+
                TABEYOperationsComp.OperationBlockToText(TNode.Node.Bank.Vault.Block(TNode.Node.Bank.BlocksCount-1).blockchainInfo)+
                ' remote:'+TABEYOperationsComp.OperationBlockToText(LBlocks[i].OperationBlock)+' Errors: '+errors);
          end;
        end else begin
          // Receiving an unexpected operationblock
          TLog.NewLog(lterror,classname,'Received a distinct block, finalizing: '+TABEYOperationsComp.OperationBlockToText(LBlocks[i].OperationBlock)+' (My block: '+TABEYOperationsComp.OperationBlockToText(TNode.Node.Bank.LastOperationBlock)+')' );
          FIsDownloadingBlocks := false;
          exit;
        end;
        sleep(1);
...
begin
  ...
  try
    ...
    try
      ...
      total_b := 0;
      repeat
        ob := TNode.Node.Bank.Vault.Block(b).blockchainInfo;
        If TABEYOperationsComp.SaveOperationBlockToStream(ob,msops) then begin
...
procedure TNetConnection.DoProcess_GetVault_Request(HeaderData: TNetHeaderData; DataBuffer: TStream);
Var _blockcount : Cardinal;
    _VaultHash : TRawBytes;
    _from,_to : Cardinal;
  sbStream : TStream;
...
begin
  {
  This call is used to obtain a chunk of the Vault
  Request:
  BlockCount (4 bytes) - The Vault checkpoint
  VaultHash (TRawBytes) - The Vaulthash of that checkpoint
  StartPos (4 bytes) - The start index (0..BlockCount-1)
  EndPos   (4 bytes) - The final index (0..BlockCount-1)
    If valid info:
      - If available will return a LZIP chunk of Vault
      - If not available (requesting for an old Vault) will retun not available
    If not valid will disconnect
  }
  DataBuffer.Read(_blockcount,SizeOf(_blockcount));
  TStreamOp.ReadAnsiString(DataBuffer,_VaultHash);
  DataBuffer.Read(_from,SizeOf(_from));
  DataBuffer.Read(_to,SizeOf(_to));
  // Protections:
  if (_from>_to) Or (_from + CT_MAX_VaultCHUNK_BLOCKS <= _to) then begin
    DisconnectInvalidClient(False,Format('Invalid GetVault values on request. From:%d to:%d',[_from,_to]));
    Exit;
  end;
  //
  sbStream := TNode.Node.Bank.Storage.CreateVaultStream(_blockcount);
  try
    responseStream := TMemoryStream.Create;
    try
      If Not Assigned(sbStream) then begin
        SendError(ntp_response,HeaderData.operation,CT_NetError_VaultNotFound,HeaderData.request_id,Format('Vault for block %d not found',[_blockcount]));
        exit;
      end;
      antPos := sbStream.Position;
      TABEYVault.LoadVaultStreamHeader(sbStream,sbHeader);
      If Not TBaseType.Equals(sbHeader.VaultHash,_VaultHash) then begin
        DisconnectInvalidClient(false,Format('Invalid Vaulthash on GetVault request (Real:%s > Requested:%s)',[TCrypto.ToHexaString(sbHeader.VaultHash),TCrypto.ToHexaString(_VaultHash)]));
        exit;
      end;
      // Response:
      sbStream.Position:=antPos;
      If not TABEYChunk.SaveVaultChunkFromVault(sbStream,responseStream,_from,_to,errors) then begin
      ...
      TLog.NewLog(ltInfo,ClassName,Format('Sending Vault(%d) chunk[%d..%d] to %s Bytes:%d',[_blockcount,_from,_to,ClientRemoteAddr,responseStream.Size]));
    finally
      responseStream.Free;
    end;
  finally
...
begin
  ...
  try
    ...
    //
    nAccounts := 0;
    sbakl := TNode.Node.Bank.Vault.OrderedAccountKeysList;
...
var op, myLastOp : TABEYOperationsComp;
    errors : String;
...
Begin
  FRemoteAccumulatedWork := 0;
  op := TABEYOperationsComp.Create(Nil);
  try
    DataBuffer.Position:=0;
    ...
    if op.LoadBlockFromStream(DataBuffer,errors) then begin
      ...
      if (DataBuffer.Size-DataBuffer.Position>=4) then begin
        ...
        if TStreamOp.ReadString(DataBuffer,other_version)>=0 then begin
          ...
          if (DataBuffer.Size-DataBuffer.Position>=SizeOf(FRemoteAccumulatedWork)) then begin
            ...
            TLog.NewLog(ltdebug,ClassName,'Received HELLO with height: '+inttostr(op.OperationBlock.block)+' Accumulated work '+IntToStr(FRemoteAccumulatedWork)+ ' Remote block: '+TABEYOperationsComp.OperationBlockToText(FRemoteOperationBlock));
          end;
        end;
        //
        if (FRemoteAccumulatedWork>TNode.Node.Bank.Vault.WorkSum) Or
          ...
          if TABEYThread.ThreadClassFound(TThreadGetNewBlockChainFromClient,nil)<0 then begin
            TThreadGetNewBlockChainFromClient.Create;
          end;
        end;
      end;
      ...
      {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,Classname,'Hello received: '+TABEYOperationsComp.OperationBlockToText(FRemoteOperationBlock));{$ENDIF}
...
begin
  ...
  try
    ...
    if not TABEYEncryption.DoABEYECIESDecrypt(TNetData.NetData.NodePrivateKey.PrivateKey,messagecrypted,decrypted) then begin
...
var operationsComp : TABEYOperationsComp;
  DoDisconnect : Boolean;
  errors : String;
  ...
  begin
    ...
    for i := 0 to High(nfpboarr) do begin
      auxOp := TABEYOperation.GetOperationFromStreamData( nfpboarr[i].opStreamData );
    ...
    If Not TAccountComp.EqualOperationBlocks(operationsComp.OperationBlock,original_OperationBlock) then begin
      ...
      TLog.NewLog(lterror,ClassName,Format('Constructed a distinct FAST PROPAGATION block with my mempool operations. Received: %s Constructed: %s',
        [TABEYOperationsComp.OperationBlockToText(original_OperationBlock),TABEYOperationsComp.OperationBlockToText(operationsComp.OperationBlock)]));
      if Not TABEYVault.IsValidOperationBlock(original_OperationBlock,errors) then begin
  ...
  try
    ...
    try
      ...
      if Not operationsComp.LoadBlockFromStream(DataBuffer,errors) then begin
        ...
        if (FRemoteAccumulatedWork>TNode.Node.Bank.Vault.WorkSum) then begin
          if (operationsComp.OperationBlock.block=TNode.Node.Bank.BlocksCount) then begin
            ...
            If Not TNode.Node.AddNewBlockChain(Self,operationsComp,bacc,errors) then begin
              ...
              if Not TABEYVault.IsValidOperationBlock(operationsComp.OperationBlock,errors) then begin
                DoDisconnect := True;
                Exit;
              end;
...
begin
  ...
  Try
    ...
    iDebugStep := 110;
    tc := TPlatform.GetTickCount;
    If TABEYThread.TryProtectEnterCriticalSection(Self,MaxWaitTime,FNetLock) then begin
      Try
        iDebugStep := 120;
                ...
                case HeaderData.operation of
                  ...
                  CT_NetOp_GetVault : Begin
                    if HeaderData.header_type=ntp_request then begin
                      ...
                      else DoProcess_GetVault_Request(HeaderData,ReceiveDataBuffer)
      ...
      if Result then begin
        ...
        if HeaderData.protocol.protocol_version>CT_NetProtocol_Available then begin
          TNode.Node.NotifyNetClientMessage(Nil,
            'Detected a higher blockchain protocol version at '+
            ClientRemoteAddr+' (v '+inttostr(HeaderData.protocol.protocol_version)+' '+inttostr(HeaderData.protocol.protocol_available)+') '+
            '... check that your version is up-to-date! Visit official website for possible updates.');
          DisconnectInvalidClient(false,Format('Invalid blockchain protocol version found: %d available: %d',[HeaderData.protocol.protocol_version,HeaderData.protocol.protocol_available]));
          Result := false;
          exit;
        end else begin
          if (FNetProtocolVersion.protocol_available>CT_NetProtocol_Available) And (Not FAlertedForNewProtocolAvailable) then begin
            ...
            TNode.Node.NotifyNetClientMessage(Nil,
              'Detected a new blockchain protocol version at '+
              ClientRemoteAddr+' (v '+inttostr(HeaderData.protocol.protocol_version)+' '+inttostr(HeaderData.protocol.protocol_available)+') '+
              '... Visit official website for possible updates.');
          end;
...
begin
  ...
  try
    ...
    TABEYOperationsComp.SaveOperationBlockToStream(TNode.Node.Bank.LastOperationBlock,data);
    ...
    data.Write(TNode.Node.Bank.Vault.WorkSum,SizeOf(TNode.Node.Bank.Vault.WorkSum));
    //
...
begin
  ...
  Try
    // Cypher message:
    TABEYEncryption.DoABEYECIESEncrypt(FClientPublicKey,TEncoding.ASCII.GetBytes(TheMessage),cyp);
...
function TNetConnection.Send_NewBlockFound(const NewBlock: TABEYOperationsComp): Boolean;
var data : TStream;
  request_id : Integer;
  netOp : Word;
  c : Cardinal;
...
begin
  ...
  Try
    ...
    If (TABEYOperationsComp.EqualsOperationBlock(FRemoteOperationBlock,NewBlock.OperationBlock)) then begin
  ...
  try
    ...
    for i := 0 to j - 1 do begin
      if TNetData.NetData.GetConnection(i,nc) then begin
        if (nc.FRemoteAccumulatedWork>maxWork) And (nc.FRemoteAccumulatedWork>TNode.Node.Bank.Vault.WorkSum) then begin
...
begin
  FTimesList := TABEYThreadList<Pointer>.Create('TNetworkAdjustedTime_TimesList');
  FTimeOffset := 0;
  FTotalCounter := 0;
end;
