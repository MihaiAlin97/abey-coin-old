...
Type
  ...
  private
    ...
    procedure SetVault(const Value: TABEYVault);
    function GetVault: TABEYVault;
  public
    Constructor Create(AOwner : TComponent); override;
    Destructor destroy; override;
    ...
    Property Vault : TABEYVault read GetVault write SetVault;
  End;
...
begin
  ...
  if Not Find(ECPrivateKey.PublicKey,Result) then begin
    ...
    P^.CryptedKey := TABEYEncryption.DoABEYAESEncrypt(raw_priv2hexa,LWalletPassword);
...
begin
  ...
  for i := 0 to FSearchableKeys.Count - 1 do begin
    ...
    if P^.HasPrivateKey then begin
      isOk := TABEYEncryption.DoABEYAESDecrypt(P^.CryptedKey, TEncoding.ASCII.GetBytes(FWalletPassword), raw );
      If isOk then begin
...
begin
  ...
  for i := 0 to FSearchableKeys.Count - 1 do begin
    ...
    If Assigned(P^.PrivateKey) then begin
      ...
      P^.CryptedKey :=  TABEYEncryption.DoABEYAESEncrypt(raw_priv2hexa,TEncoding.ASCII.GetBytes(FWalletPassword));
    end else begin
...
function TWalletKeysExt.GetVault: TABEYVault;
begin
  Result := Nil;
  if Assigned(FOrderedAccountKeysList) then begin
    Result := FOrderedAccountKeysList.Vault;
  end;
end;

procedure TWalletKeysExt.SetVault(const Value: TABEYVault);
Var i : Integer;
begin
  if Assigned(FOrderedAccountKeysList) then begin
    if FOrderedAccountKeysList.Vault<>Value then FreeAndNil(FOrderedAccountKeysList)
    else exit;
  end;
...
begin
  try
    ...
    FKeys.WalletFileName := TFolderHelper.GetABEYDataFolder+PathDelim+'ABEY.keys';
  except
    on E:Exception do begin
      E.Message := 'Cannot open your wallet... Perhaps another instance of the wallet is active!'+#10+#10+E.Message;
      Raise;
    end;
  end;
end;
...
begin
  Result := TABEYEncryption.DoABEYAESEncrypt(AKey.PrivateKey.ExportToRaw,TEncoding.ANSI.GetBytes(APassword)).ToHexaString;
end;
...
begin
  ...
  decrypt := Nil;
  if TABEYEncryption.DoABEYAESDecrypt(TCrypto.HexaToRaw(AKeyHexString),TEncoding.ANSI.GetBytes(AKeyPassword),decrypt) then begin
