...
{
  This unit is a bridge between blockchain and CryptoLib4Pascal in order to
...
Uses SysUtils, UBaseTypes, UPCDataTypes,
  ...
  ClpIABEYECIESKdfBytesGenerator,
  ClpIABEYIESEngine
  ;

Type
  TABEYCryptoLib4Pascal = Class
  strict private
  const
  ...
  strict private
    ...
    class var FABEYIESEngine : IABEYIESEngine;
    class var FIESParameterSpec : IIESParameterSpec;
    class constructor TABEYCryptoLib4Pascal();
  ...
  public
    ...
    class function DoABEYECIESDecrypt(const AEC_OpenSSL_NID : Word; const APrivateKey, AEncryptedMessage : TBytes; var ADecryptedMessage : TBytes) : Boolean;
...
{ TABEYCryptoLib4Pascal }

class function TABEYCryptoLib4Pascal.DoECDSASign(const AEC_OpenSSL_NID : Word;
...
class function TABEYCryptoLib4Pascal.DoECDSAVerify(const APublicKey: TECDSA_Public; const AMessage: TBytes; const ASignature: TECDSA_SIG): Boolean;
var
  LDomain: IECDomainParameters;
...
class function TABEYCryptoLib4Pascal.DoGetPublicKey(const AEC_OpenSSL_NID: Word; const APrivateKey: TBytes): TECDSA_Public;
var
  LDomain: IECDomainParameters;
  LPrivD : TBigInteger;
...
class function TABEYCryptoLib4Pascal.DoGetRandomPrivateKey(const AEC_OpenSSL_NID: Word): TBytes;
var
  LDomain: IECDomainParameters;
  LCurve: IX9ECParameters;
...
class function TABEYCryptoLib4Pascal.DoABEYAESDecrypt(
...
class function TABEYCryptoLib4Pascal.DoABEYAESEncrypt(const AMessage, APassword: TBytes): TBytes;
var
...
class function TABEYCryptoLib4Pascal.DoABEYECIESDecrypt(
...
begin
  try
    ...
    // Decryption
    LCipherDecrypt := TIESCipher.Create(FABEYIESEngine);
...
class function TABEYCryptoLib4Pascal.DoABEYECIESEncrypt(
...
begin
  ...
  LPubKeyParams := TECPublicKeyParameters.Create('ECDSA', LPoint, LDomain);
  // Encryption
  LCipherEncrypt := TIESCipher.Create(FABEYIESEngine);
  LCipherEncrypt.Init(True, LPubKeyParams, FIESParameterSpec, FRandom);
  AEncryptedMessage := LCipherEncrypt.DoFinal(AMessage);
  Result := True;
end;

class procedure TABEYCryptoLib4Pascal.DoRIPEMD160(const AInput: TBytes; var AOutput: TBytes);
...
class procedure TABEYCryptoLib4Pascal.DoSHA256(const AInput: TBytes; var AOutput: TBytes);
var Lsha : TSHA2_256;
begin
...
class function TABEYCryptoLib4Pascal.EVP_GetKeyIV(const APasswordBytes,
...
class function TABEYCryptoLib4Pascal.EVP_GetSalt: TBytes;
begin
...
class function TABEYCryptoLib4Pascal.GetCurveAndDomainParameters(
...
class function TABEYCryptoLib4Pascal.GetDomainParameters(const AEC_OpenSSL_NID: Word): IECDomainParameters;
begin
  case AEC_OpenSSL_NID of
...
class constructor TABEYCryptoLib4Pascal.TABEYCryptoLib4Pascal;
    ...
    begin
      // Set up IES Cipher Parameters For Compatibility With Current Implementation
    ...
    function GetECIESABEYCompatibilityEngine(): IABEYIESEngine;
    var
      cipher: IBufferedBlockCipher;
      AesEngine: IAesEngine;
      blockCipher: ICbcBlockCipher;
      ECDHBasicAgreementInstance: IECDHBasicAgreement;
      KDFInstance: IABEYECIESKdfBytesGenerator;
      DigestMACInstance: IMac;

    begin
      // Set up IES Cipher Engine For Compatibility With Blockchain

      ECDHBasicAgreementInstance := TECDHBasicAgreement.Create();

      KDFInstance := TABEYECIESKdfBytesGenerator.Create
      ...
      Result := TABEYIESEngine.Create(ECDHBasicAgreementInstance, KDFInstance,
...
begin
  ...
  // Init ECIES
  FABEYIESEngine := GetECIESABEYCompatibilityEngine;
