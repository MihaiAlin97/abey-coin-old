...
Type
  ...
  private
    FNodeLog : TLog;
    FLockMempool : TABEYCriticalSection;
    FOperationSequenceLock : TABEYCriticalSection;
    FNotifyList : TList<TNodeNotifyEvents>;
    FBank : TABEYBank;
    FMemPoolOperationsComp : TABEYOperationsComp;
    FMemPoolAddingOperationsList : TOrderedRawList;
    FNetServer : TNetServer;
    FBCBankNotify : TABEYBankNotify;
    FPeerCache : String;
  ...
  public
    ...
    Property Bank : TABEYBank read FBank;
    ...
    // Return Account based on current state (Vault + Mempool operations)
    function GetMempoolAccount(AAccountNumber : Cardinal) : TAccount;
    // Locking methods to access to the Mempool
    function LockMempoolRead : TABEYOperationsComp;
    procedure UnlockMempoolRead;
    function LockMempoolWrite : TABEYOperationsComp;
    procedure UnlockMempoolWrite;
    //
    Function AddNewBlockChain(SenderConnection : TNetConnection; NewBlockOperations: TABEYOperationsComp; var newBlockAccount: TBlockAccount; var errors: String): Boolean;
    ...
    Function AddOperation(SenderConnection : TNetConnection; Operation : TABEYOperation; var errors: String): Boolean;
    ...
    //
    procedure GetStoredOperationsFromAccount(AOwnerThread : TABEYThread; const OperationsResume: TList<TOperationResume>; account_number: Cardinal; MaxDepth, StartOperation, EndOperation : Integer; SearchBackwardsStartingAtBlock : Cardinal=0); overload;
    ...
    Function FindOperation(Const OperationComp : TABEYOperationsComp; Const OperationHash : TRawBytes; var block : Cardinal; var operation_block_index : Integer) : Boolean;
    Function FindOperationExt(Const OperationComp : TABEYOperationsComp; Const OperationHash : TRawBytes; var block : Cardinal; var operation_block_index : Integer) : TSearchOperationResult;
    ...
    //
    Procedure InitVaultAndOperations(max_block_to_read : Cardinal = $FFFFFFFF; restoreProgressNotify : TProgressNotify = Nil);
    ...
    Property OperationSequenceLock : TABEYCriticalSection read FOperationSequenceLock;
    ...
    procedure MarkVerifiedECDSASignaturesFromMemPool(newOperationsToValidate : TABEYOperationsComp);
    class function NodeVersion : String;
  End;

  TThreadSafeNodeNotifyEvent = Class(TABEYThread)
  ...
  private
    ...
    FPendingNotificationsList : TABEYThreadList<Pointer>;
  ...
  TThreadNodeNotifyNewBlock = Class(TABEYThread)
    FNetConnection : TNetConnection;
    FSanitizedOperationsHashTree : TOperationsHashTree;
    FNewBlockOperations : TABEYOperationsComp;
  protected
    procedure BCExecute; override;
  public
    Constructor Create(NetConnection : TNetConnection; MakeACopyOfNewBlockOperations: TABEYOperationsComp; MakeACopyOfSanitizedOperationsHashTree : TOperationsHashTree);
    destructor Destroy; override;
  End;

  TThreadNodeNotifyOperations = Class(TABEYThread)
    FNetConnection : TNetConnection;
  protected
    procedure BCExecute; override;
  public
...
function TNode.AddNewBlockChain(SenderConnection: TNetConnection; NewBlockOperations: TABEYOperationsComp;
...
begin
  ...
  if FDisabledsNewBlocksCount>0 then begin
    ...
    sClientRemoteAddr,TABEYOperationsComp.OperationBlockToText(NewBlockOperations.OperationBlock)]));
  ...
  Try
    ...
    TLog.NewLog(ltdebug,Classname,Format('Starting AddNewBlockChain %d Operations %d from %s NewBlock:%s',[
      OpBlock.block,NewBlockOperations.Count,sClientRemoteAddr,TABEYOperationsComp.OperationBlockToText(OpBlock)]));
    If Not TABEYThread.TryProtectEnterCriticalSection(Self,5000,FLockMempool) then begin
    ...
    try
      // Check block number:
      if TABEYOperationsComp.EqualsOperationBlock(Bank.LastOperationBlock,NewBlockOperations.OperationBlock) then begin
        ...
        Try
          ...
          While (opsht.OperationsCount<maxResend) And (i<LLockedMempool.Count) do begin
            ...
            if (j=0) Or (j<=minBlockResend) then begin
              ...
              {$IFDEF HIGHLOG}TLog.NewLog(ltInfo,ClassName,'Sanitized operation not included to resend (j:'+IntToStr(j)+'>'+inttostr(minBlockResend)+') ('+inttostr(i+1)+'/'+inttostr(opsht.OperationsCount)+'): '+opsht.GetOperation(i).ToString);{$ENDIF}
            end;
            inc(i);
          end;
...
function TNode.AddOperation(SenderConnection : TNetConnection; Operation: TABEYOperation; var errors: String): Boolean;
var ops : TOperationsHashTree;
begin
  TLog.NewLog(ltInfo,ClassName,'AddOperation');
...
function TNode.AddOperations(SenderConnection : TNetConnection; AOperationsHashTreeToAdd : TOperationsHashTree; OperationsResult : TOperationsResumeList; var errors: String): Integer;
  {$IFDEF BufferOfFutureOperations}
  Procedure Process_BufferOfFutureOperations(ALockedMempool : TABEYOperationsComp; valids_operations : TOperationsHashTree);
  Var i,j, nAdded, nDeleted : Integer;
    sAcc : TAccount;
    ActOp : TABEYOperation;
    e : String;
  Begin
    ...
    For j:=0 to 3 do begin
      ...
      While (i<FBufferAuxWaitingOperations.OperationsCount) do begin
        ...
        If ALockedMempool.AddOperation(true,ActOp,e) then begin
          ...
          sAcc := ALockedMempool.VaultTransaction.Account(ActOp.SignerAccount);
...
Var
  ...
  LLockedMempool : TABEYOperationsComp;
  LOpsToAdd : TList<TABEYOperation>;
  LTempVaultTransaction : TABEYVaultTransaction;
...
begin
  ...
  try
    LOpsToAdd := TList<TABEYOperation>.Create;
    try
      ...
      // Step 2:
      LTempVaultTransaction := TABEYVaultTransaction.Create(Bank.Vault);
      try
        TABEYOperationsSignatureValidator.MultiThreadPreValidateSignatures(LTempVaultTransaction,LOpsToAdd,Nil);
      finally
        LTempVaultTransaction.Free;
      end;
      ...
      for j := 0 to LOpsToAdd.Count-1 do begin
        ...
        try
          ...
          If (LLockedMempool.OperationsHashTree.IndexOfOperation(ActOp)<0) then begin
            ...
            If (ActOp.OperationFee=0) And (Bank.Vault.CurrentProtocol>=CT_PROTOCOL_2) And
              ...
              if Assigned(OperationsResult) then begin
                TABEYOperation.OperationToOperationResume(0,ActOp,True,ActOp.SignerAccount,OPR);
                OPR.valid := false;
              ...
              if (LLockedMempool.AddOperation(true,ActOp,e)) then begin
                ...
                if Assigned(OperationsResult) then begin
                  ...
                  OPR.Balance := LLockedMempool.VaultTransaction.Account(ActOp.SignerAccount).balance;
                  OperationsResult.Add(OPR);
                end;
              end else begin
                inc(nError);
                ...
                If (Assigned(SenderConnection)) Then begin
                  if ActOp.SignerAccount<LLockedMempool.VaultTransaction.FreezedVault.AccountsCount then begin
                    sAcc := LLockedMempool.VaultTransaction.Account(ActOp.SignerAccount);
                    If (sAcc.n_operation<ActOp.N_Operation) Or
...
begin
  ...
  FLockMempool := TABEYCriticalSection.Create('TNode_LockMempool');
  FOperationSequenceLock := TABEYCriticalSection.Create('TNode_OperationSequenceLock');
  FBank := TABEYBank.Create(Self);
  FBCBankNotify := TABEYBankNotify.Create(Self);
  FBCBankNotify.Bank := FBank;
  FBCBankNotify.OnNewBlock := OnBankNewBlock;
  FNetServer := TNetServer.Create;
  FMemPoolOperationsComp := TABEYOperationsComp.Create(Nil);
...
begin
  Result := TABEYThread.TryProtectEnterCriticalSection(Self,MaxWaitMilliseconds,FLockMempool);
end;
...
procedure TNode.MarkVerifiedECDSASignaturesFromMemPool(newOperationsToValidate: TABEYOperationsComp);
      ...
      if Not TNetData.NetData.IsGettingNewBlockChainFromClient(CurrentProcess) then begin
        ...
        try
          if TNetData.NetData.MaxRemoteOperationBlock.block>LLockedMempool.OperationBlock.block then begin
            CurrentProcess := 'Downloading block '+inttostr(TNetData.NetData.MaxRemoteOperationBlock.block)+'...';
          end else begin
            CurrentProcess := '';
            Result := true;
          end;
...
procedure TNode.GetStoredOperationsFromAccount(AOwnerThread : TABEYThread; const OperationsResume: TList<TOperationResume>; account_number: Cardinal;
  ...
  begin
    ...
    opc := TABEYOperationsComp.Create(Nil);
    Try
      ...
      try
        ...
        while (last_block_number>block_number) And (act_depth>0)
          ...
          for i := l.Count - 1 downto 0 do begin
            ...
            If TABEYOperation.OperationToOperationResume(block_number,Op,False,account_number,OPR) then begin
          ...
          if (TAccountComp.AccountBlock(account_number)=block_number) then begin
            TABEYProtocol.GetRewardDistributionForNewBlock(opc.OperationBlock,acc_0_miner_reward,acc_4_dev_reward,acc_4_for_dev);
...
begin
  ...
  if account_number>=Bank.Vault.AccountsCount then Exit;
  if StartOperation>EndOperation then Exit;
  acc := Bank.Vault.Account(account_number);
...
var i : Integer;
  ...
  OperationComp : TABEYOperationsComp;
  opr : TOperationResume;
...
begin
  ...
  LLockedMempool := LockMempoolRead;
  try
    n_operation := LLockedMempool.VaultTransaction.Account(account).n_operation;
  finally
    UnlockMempoolRead;
  end;
  ...
  If (block=0) then begin
    ...
    try
      For i:=LLockedMempool.Count-1 downto 0 do begin
        ...
        If (op.IsSignerAccount(account)) then begin
          ...
          If (found_n_operation<=n_operation) then begin
            TABEYOperation.OperationToOperationResume(0,op,False,account,opr);
            opr.Balance:=-1;
      ...
      block := Bank.Vault.Account(account).updated_block;
    finally
      UnlockMempoolRead;
    end;
  end;
  // Search in previous blocks
  OperationComp := TABEYOperationsComp.Create(Nil);
  Try
      ...
      For i:=OperationComp.Count-1 downto 0 do begin
        ...
        if (op.IsSignerAccount(account)) then begin
          If (n_operation_high=n_operation_low) and (op.GetAccountN_Operation(account)=n_operation) // If searching only 1 n_operation, n_operation must match
            ...
            TABEYOperation.OperationToOperationResume(block,op,True,account,opr);
            opr.time:=Bank.Vault.Block(block).blockchainInfo.timestamp;
...
procedure TNode.InitVaultAndOperations(max_block_to_read : Cardinal = $FFFFFFFF; restoreProgressNotify : TProgressNotify = Nil);
...
function TNode.FindOperationExt(const OperationComp: TABEYOperationsComp;
...
begin
  ...
  If not TABEYOperation.DecodeOperationHash(OperationHash,block,account,n_operation,md160) then exit;
  initial_block := block;
  //
  If (account>=Bank.AccountsCount) then exit; // Invalid account number
  ...
  if (block=0) then begin
      ...
      Try
        For i:=0 to LLockedMempool.Count-1 do begin
          ...
          If (op.SignerAccount=account) then begin
            opHashValid := TABEYOperation.OperationHashValid(op,0);
            opHash_OLD := TABEYOperation.OperationHash_OLD(op,0);
    ...
    block := Bank.Vault.Account(account).updated_block;
    if Bank.Vault.Account(account).n_operation<n_operation then exit; // n_operation is greater than found in Vault
  end;
    ...
    For i:=OperationComp.Count-1 downto 0 do begin
      ...
      if (op.IsSignerAccount(account)) then begin
        ...
        If (aux_n_op=n_operation) then begin
          ...
          opHashValid := TABEYOperation.OperationHashValid(op,initial_block);
          If (TBaseType.Equals(opHashValid,OperationHash)) then begin
            operation_block_index:=i;
            Result := found;
            exit;
          end else if (block<CT_Protocol_Upgrade_v2_MinBlock) then begin
            opHash_OLD := TABEYOperation.OperationHash_OLD(op,initial_block);
...
function TNode.FindOperation(const OperationComp: TABEYOperationsComp;
  const OperationHash: TRawBytes; var block: Cardinal;
...
begin
  LLockedMempool := LockMempoolRead;
  try
    Result := LLockedMempool.VaultTransaction.Account(AAccountNumber);
  finally
    UnlockMempoolRead;
  end;
end;

function TNode.LockMempoolRead: TABEYOperationsComp;
begin
...
function TNode.LockMempoolWrite: TABEYOperationsComp;
begin
...
begin
  Result := false;
  if Not TABEYThread.TryProtectEnterCriticalSection(Self,4000,FLockMempool) then begin
...
begin
  ...
  FPendingNotificationsList := TABEYThreadList<Pointer>.Create('TNodeNotifyEvents_PendingNotificationsList');
...
constructor TThreadNodeNotifyNewBlock.Create(NetConnection: TNetConnection; MakeACopyOfNewBlockOperations: TABEYOperationsComp; MakeACopyOfSanitizedOperationsHashTree : TOperationsHashTree);
begin
  ...
  FNewBlockOperations := TABEYOperationsComp.Create(Nil);
