...
{
  This unit adds a Extra available messages to blockchain protocol
...
type
  TABEYTNetDataExtraMessages = Class
  private
    FNode : TNode;
    FNetData : TNetData;
    ...
    function RandomGetWalletKeysAccount(const AVault : TABEYVault; AWalletKeys: TWalletKeysExt; AMinBalance, AMaxBalance : Int64; out AKey: Integer; out AAccount: TAccount): Boolean;
  protected
  public
    class function InitNetDataExtraMessages(ANode : TNode; ANetData : TNetData; AWalletKeys : TWalletKeysExt) : TABEYTNetDataExtraMessages;
...
var _PCTNetDataExtraMessages : TABEYTNetDataExtraMessages = Nil;

{ TABEYTNetDataExtraMessages }

class function TABEYTNetDataExtraMessages.AskForFreeAccount(ANewPubliKey : TAccountKey): Integer;
begin
...
constructor TABEYTNetDataExtraMessages.Create(ANode : TNode; ANetData : TNetData; AWalletKeys : TWalletKeysExt);
begin
  FNode := ANode;
...
destructor TABEYTNetDataExtraMessages.Destroy;
begin
...
function TABEYTNetDataExtraMessages.DoAskForFreeAccount(ANewPubliKey : TAccountKey): Integer;
var i : Integer;
...
{$IFDEF TESTNET}
procedure TABEYTNetDataExtraMessages.DoGiveMeAnAccount(ANetData: TNetData;
...
begin
  ...
  if Not RandomGetWalletKeysAccount(FNode.Bank.Vault,FWalletKeys,0,10000,LIndexKey,LAccount) then Exit;
  // Send
  LPayload.FromString('Free Account to '+ASenderConnection.Client.RemoteHost);
  LOpChangeKey := TOpChangeKey.Create(FNode.Bank.Vault.CurrentProtocol,LAccount.account,LAccount.n_operation+1,
...
procedure TABEYTNetDataExtraMessages.DoGiveMeMoney(ANetData: TNetData;
  ASenderConnection: TNetConnection; const AHeaderData: TNetHeaderData;
  AReceivedData, AResponseData: TStream);
...
begin
  ...
  if Not RandomGetWalletKeysAccount(FNode.Bank.Vault,FWalletKeys,1000,-1,LIndexKey,LAccount) then Exit;
  ...
  LOpTransaction := TOpTransaction.CreateTransaction(FNode.Bank.Vault.CurrentProtocol,LAccount.account,LAccount.n_operation+1,
...
class function TABEYTNetDataExtraMessages.InitNetDataExtraMessages(ANode: TNode;
  ANetData: TNetData; AWalletKeys: TWalletKeysExt): TABEYTNetDataExtraMessages;
begin
  if not Assigned(_PCTNetDataExtraMessages) then begin
    _PCTNetDataExtraMessages := TABEYTNetDataExtraMessages.Create(ANode,ANetData,AWalletKeys);
  end;
  Result := _PCTNetDataExtraMessages;
end;

procedure TABEYTNetDataExtraMessages.OnTNetDataProcessReservedAreaMessage(ANetData : TNetData; ASenderConnection : TNetConnection; const AHeaderData : TNetHeaderData; AReceivedData : TStream; AResponseData : TStream);
begin
...
function TABEYTNetDataExtraMessages.RandomGetWalletKeysAccount(
  const AVault: TABEYVault; AWalletKeys: TWalletKeysExt; AMinBalance,
...
begin
  ...
  if AVault.AccountsCount>0 then L0Account := AVault.Account(0)
  ...
  Repeat
    if (AWalletKeys.Key[AKey].HasPrivateKey) And (Assigned(AWalletKeys.Key[AKey].PrivateKey)) And (AWalletKeys.AccountsKeyList.AccountKeyList[AKey].Count>0) then begin
      ...
      Repeat
        AAccount := AVault.Account( AWalletKeys.AccountsKeyList.AccountKeyList[AKey].Get( iInt ) );
        //
