...
uses
  ...
  UPCHardcodedRandomHashTable, UPCEncryption,lazlogger,
...
Type
  ...
  TAccountState = (as_Unknown, as_Normal, as_ForSale, as_ForContract);  //TODO rename as_ForContract to something more meaningful

  {TContract = Record          //TODO think of TContract structure
    id: Cardinal;
    operation_id: Cardinal;
  end; }
  ...
  TAccountInfo = Record
    ...
    //contracts : array of TContract;   //TODO: see if they can be changed dynamically
  end;
  ...
  TOperationBlock = Record
    ...
    initial_safe_box_hash: TRawBytes; // RAW Vault Hash value (32 bytes, it's a Sha256)
  ...
  TABEYProtocol = Class
  private
    FPCHardcodedRandomHashTable : TABEYHardcodedRandomHashTable;
  public
    constructor Create;
  ...
  //TODO:      ADD UTFAB ( Utility Token for ABEY blockchain for blockchain operations
  //TODO:      this should be a stable coin available only to convert to/from ABEY and should be used to pay blockchain utilities at a constant price
  //TODO: Think of the following : If this is to be a stable coin, one safe way to achieve this without pumping/dumping money in this should be if it's unlisted and exchangeable only with ABEY
  //TODO: This means that ABEY coins will either: be deleted from blockchain(bad) or be transfered to a reserve fund owned by ABEY foundation
  TAccount = Record
  ...
  {
    Protocol 2:
    Introducing OperationBlock info on the Vault, this will allow checkpointing a Vault because
    each row of the Vault (TBlockAccount) will have data about how to calculate
    ...
    If someone wants to change the Vault and spam, will need to find values
    to alter Vault accounts of last checkpoint and also find new blocks prior
  ...
  Private
    FAutoAddAll : Boolean;
    FAccountList : TABEYVault;
    FOrderedAccountKeysList : TABEYThreadList<Pointer>; // An ordered list of pointers to quickly find account keys in account list
  ...
  public
    Constructor Create(AccountList : TABEYVault; AutoAddAll : Boolean);
    Destructor Destroy; override;
    ...
    Property Vault : TABEYVault read FAccountList;
    Procedure Clear;
  ...
  // Vault is a box that only can be updated using VaultTransaction, and this
  // happens only when a new BlockChain is included. After this, a new "VaultHash"
  // is created, so each Vault has a unique VaultHash

  TABEYVaultTransaction = Class;
  ...
  TABEYVault = Class
  private
    FBlockAccountsList : TList<Pointer>; // Used when has no PreviousVault
    FModifiedBlocksSeparatedChain : TOrderedBlockAccountList; // Used when has PreviousVault (Used if we are on a Separated chain)
    //
    // OrderedAccountKeysList (Added after Build 3.0.1) allows an indexed search of public keys in the Vault with mem optimization
    ...
    FTotalBalance: Int64;
    FVaultHash : TRawBytes;
    FLock: TABEYCriticalSection; // Thread safe
    FWorkSum : UInt64;
    FCurrentProtocol: Integer;
    // Snapshots utility new on V3
    FSnapshots : TList<Pointer>; // Will save a Snapshots lists in order to rollback Vault to a previous block state
    FMaxVaultSnapshots : Integer;
    ...
    FAddedNamesSincePreviousVault : TOrderedRawList;
    FDeletedNamesSincePreviousVault : TOrderedRawList;
    // Is capturing data from a snapshot?
    FPreviousVault : TABEYVault;  // PreviousVault is the Vault with snpashots where this Vault searches
    FPreviousVaultOriginBlock : Integer;
    // Has chains based on this Vault?
    FSubChains : TList<TABEYVault>; // Will link to subchains (other Vault) based on a current snapshot of this Vault
    //
  ...
  public
    Constructor Create;
    Destructor Destroy; override;
    procedure SetToPrevious(APreviousVault : TABEYVault; StartBlock : Cardinal);
    ...
    Procedure CopyFrom(accounts : TABEYVault);
    ...
    Function LoadVaultFromStream(Stream : TStream; checkAll : Boolean; var LastReadBlock : TBlockAccount; var errors : String) : Boolean; overload;
    Function LoadVaultFromStream(Stream : TStream; checkAll : Boolean; checkVaultHash : TRawBytes; progressNotify : TProgressNotify; previousCheckedVault : TABEYVault; var LastReadBlock : TBlockAccount; var errors : String) : Boolean; overload;
    Class Function LoadVaultStreamHeader(Stream : TStream; var sbHeader : TABEYVaultHeader) : Boolean;
    Class Function SaveVaultStreamHeader(Stream : TStream; protocol : Word; OffsetStartBlock, OffsetEndBlock, CurrentVaultBlocksCount : Cardinal) : Boolean;
    Class Function MustVaultBeSaved(BlocksCount : Cardinal) : Boolean;
    Class Function InitialVaultHash : TRawBytes;
    Procedure SaveVaultBlockToAStream(DestStream : TStream; nBlock : Cardinal);
    Procedure SaveVaultToAStream(Stream : TStream; FromBlock, ToBlock : Cardinal);
    class Function CopyVaultStream(Source,Dest : TStream; FromBlock, ToBlock : Cardinal; var errors : String) : Boolean;
    class Function ConcatVaultStream(Source1, Source2, Dest : TStream; var errors : String) : Boolean;
    class function ValidAccountName(const new_name : TRawBytes; var errors : String) : Boolean;

    Function IsValidNewOperationsBlock(Const newOperationBlock : TOperationBlock; checkVaultHash, checkValidOperationsBlock : Boolean; var errors : String) : Boolean;
    ...
    Function CalcVaultHash : TRawBytes;
    ...
    Property VaultHash : TRawBytes read FVaultHash;
    ...
    Property PreviousVaultOriginBlock : Integer Read FPreviousVaultOriginBlock;
  ...
  public
    Constructor Create(Vault : TABEYVault);
    Destructor Destroy; override;
    ...
    Property FreezedVault : TABEYVault read FFreezedAccounts;
    Property TotalFee : Int64 read FTotalFee;
    Property TotalBalance : Int64 read FTotalBalance;
    Procedure CopyFrom(transaction : TABEYVaultTransaction);
...
Const
  //CT_Contract_Nul: TContract = (id:0;operation_id:0);
  ...
  CT_VaultChunkIdentificator = 'SafeBoxChunk';
  CT_PCVaultHeader_NUL : TABEYVaultHeader = (protocol:0;startBlock:0;endBlock:0;blocksCount:0;VaultHash:Nil);

function Check_Vault_Names_Consistency(sb : TABEYVault; const title :String; var errors : String) : Boolean;
Procedure Check_Vault_Integrity(sb : TABEYVault; title: String);
...
Begin
  ...
  Try
    ...
    If (errors<>'') then begin
      errors := title+' '+errors;
      Result := False;
      TLog.NewLog(lterror,'Check_Vault_Names_Consistency',errors);
    end;
  finally
    TLog.NewLog(ltDebug,'Check_Vault_Names_Consistency','Used time '+IntToStr(TPlatform.GetElapsedMilliseconds(tc))+' milliseconds');
  end;
end;
...
var _INTERNAL_ABEYProtocol : TABEYProtocol = Nil;

class function TABEYProtocol.GetNewTarget(vteorical, vreal: Cardinal; protocol_version : Integer; isSlowMovement : Boolean; const actualTarget: TRawBytes): TRawBytes;
Var
  bnact, bnaux: TBigNum;
...
class procedure TABEYProtocol.CalcProofOfWork_Part1(const operationBlock: TOperationBlock; out Part1: TRawBytes);
...
class procedure TABEYProtocol.CalcProofOfWork_Part3(const operationBlock: TOperationBlock; out Part3: TRawBytes);
var ms : TMemoryStream;
begin
  ms := TMemoryStream.Create;
  try
...
class function TABEYProtocol.AllowUseHardcodedRandomHashTable(
  const AHardcodedFileName: String;
  const AHardcodedSha256Value: TRawBytes): Boolean;
var LTmp : TABEYHardcodedRandomHashTable;
...
begin
  ...
  LTmp := TABEYHardcodedRandomHashTable.Create;
  try
    ...
    try
      if LTmp.LoadFromStream(LFileStream,LInternalHardcodedSha256) then begin
        if TBaseType.Equals(LInternalHardcodedSha256, AHardcodedSha256Value) then begin
          if Not Assigned(_INTERNAL_ABEYProtocol) then begin
            _INTERNAL_ABEYProtocol := TABEYProtocol.Create;
          end;
          _INTERNAL_ABEYProtocol.FPCHardcodedRandomHashTable.CopyFrom(LTmp);
          TLog.NewLog(ltinfo,ClassName,Format('Added %d (%d) Hardcoded RandomHash from file:%s (%s)',
            [LTmp.Count,_INTERNAL_ABEYProtocol.FPCHardcodedRandomHashTable.Count,
...
constructor TABEYProtocol.Create;
begin
  FPCHardcodedRandomHashTable := TABEYHardcodedRandomHashTable.Create;
end;

destructor TABEYProtocol.Destroy;
begin
  FreeAndNil(FPCHardcodedRandomHashTable);
  inherited;
end;

class procedure TABEYProtocol.CalcProofOfWork(const operationBlock: TOperationBlock; out PoW: TRawBytes);
var ms : TMemoryStream;
  accKeyRaw : TRawBytes;
  LDigest : TRawBytes;
begin
  ...
  try
    ...
    if CT_ACTIVATE_RANDOMHASH_V4 AND (operationBlock.protocol_version >= CT_PROTOCOL_4) then begin
      if Assigned(_INTERNAL_ABEYProtocol) then begin
        SetLength(LDigest,ms.Size);
        Move(ms.Memory^,LDigest[0],ms.Size);
        if _INTERNAL_ABEYProtocol.FPCHardcodedRandomHashTable.FindRandomHashByDigest(LDigest,PoW) then Exit;
      end;
      TCrypto.DoRandomHash(ms.Memory,ms.Size,PoW);
    end else
      TCrypto.DoDoubleSha256WithABEY(ms.Memory,ms.Size,PoW);
  finally
    ms.Free;
  end;
end;

class function TABEYProtocol.IsValidMinerBlockPayload(const newBlockPayload: TRawBytes): Boolean;
var i : Integer;
begin
  Result := False;
...
class procedure TABEYProtocol.GetRewardDistributionForNewBlock(const OperationBlock : TOperationBlock; out acc_0_miner_reward, acc_4_dev_reward : Int64; out acc_4_for_dev : Boolean);
begin
...
class function TABEYProtocol.GetRewardForNewLine(line_index: Cardinal): UInt64;
Var n, i : Cardinal;
begin
  {$IFDEF TESTNET}
  ...
  {$ELSE}
  n := (line_index - CT_Milestone1BlockNumber) DIV CT_NewLineRewardDecrease; // FOR PRODUCTION
  {$ENDIF}
  if (line_index < CT_Milestone1BlockNumber) then
    Result := CT_Milestone1Reward
  else begin
...
class function TABEYProtocol.MinimumTarget(protocol_version: Integer): Cardinal;
begin
...
class function TABEYProtocol.ResetTarget(current_target: Cardinal; protocol_version: Integer): Cardinal;
begin
...
class function TABEYProtocol.TargetFromCompact(encoded: Cardinal; protocol_version : Integer): TRawBytes;
Var
...
class function TABEYProtocol.TargetToCompact(target: TRawBytes; protocol_version : Integer): Cardinal;
Var
  bn, bn2: TBigNum;
  i: Int64;
...
begin
  ...
  if (Length(value)>(50*1000*1000)) then begin
    TLog.NewLog(lterror,Classname,'Invalid stream size! '+Inttostr(Length(value)));
    raise Exception.Create('Invalid stream size! '+Inttostr(Length(value)));
  end;
...
begin
  case AccountInfo.state of
    ...
    as_ForContract: AccountKey2RawString(AccountInfo.accountKey,dest);
    as_ForSale: begin
      ms := TMemoryStream.Create;
      Try
    ...
    raise Exception.Create('[ABEY-DEV] AccountInfo2RawString() error');
  end;
...
class function TAccountComp.AccountTxtNumberToAccountNumber(const account_txt_number: String; var account_number: Cardinal): Boolean;
  // Account number can be a simple number "123456" or a number with checksum separated by "-","." or "_"
...
begin
  case EC_OpenSSL_NID of
    CT_NID_secp256k1 : begin
      Result := 'Normal (256-bit, same as Bitcoin)';
    end;
    CT_NID_secp384r1 : begin
      Result := 'Strong (384-bit, NIST P-384)';
    end;
    CT_NID_sect283k1 : Begin
      Result := 'Stronger (283-bit, NIST K-283)';
    End;
    CT_NID_secp521r1 : begin
      Result := 'Strongest (521-bit, NIST P-521)';
    end
...
begin
  Result := ((AccountInfo.state=as_ForSale) And ((AccountInfo.locked_until_block)>=blocks_count)) or ((AccountInfo.state=as_ForContract) And ((AccountInfo.locked_until_block)>=blocks_count));
  if AccountInfo.state=as_ForContract then begin
    DebugLn('TAccountComp.IsAccountLocked state: ','as_ForContract');
    DebugLn('TAccountComp.IsAccountLocked locked_until_block: ',IntToStr(AccountInfo.locked_until_block));
    DebugLn('TAccountComp.IsAccountLocked blocks_count: ',IntToStr(blocks_count));
    DebugLn('result: ',BoolToStr(Result));

    if Result = True then DebugLn('true')
    else  DebugLn('false');
  end;
end;
...
begin
  ...
  case accountInfo.state of
    ...
    as_ForContract: begin
      Result := IsValidAccountKey(accountInfo.accountKey,errors);
    end
  else
    raise Exception.Create('[ABEY-DEV] IsValidAccountInfo() error');
  end;
end;
...
begin
  ...
  Try
    ...
    case w of
      ...
      raise Exception.Create('[ABEY-DEV] RawString2AccountInfo() error');
    end;
  Finally
    ms.Free;
  end;
...
Type
  TVaultSnapshot = Record
    ...
    oldTotalFee: Int64;
    oldVaultHash : TRawBytes;
    oldWorkSum : UInt64;
    oldCurrentProtocol: Integer;
  end;
  PVaultSnapshot = ^TVaultSnapshot;

Const
  CT_TVaultSnapshot_NUL : TVaultSnapshot = (nBlockNumber : 0; oldBlocks : Nil; newBlocks : Nil; namesDeleted : Nil; namesAdded : Nil;oldBufferBlocksHash:Nil;oldTotalBalance:0;oldTotalFee:0;oldVaultHash:Nil;oldWorkSum:0;oldCurrentProtocol:0);

function TABEYVault.Account(account_number: Cardinal): TAccount;
var
  iBlock : Integer;
...
begin
  ...
  try
    ...
    If (Assigned(FPreviousVault)) then begin
...
function TABEYVault.AddNew(const blockChain: TOperationBlock): TBlockAccount;
{ PIP-0011 (dev reward) workflow: (** Only on V3 protocol **)
  ...
  - Example:
    ...
    - blockChain.reward = 50 tokens
    - blockChain.fee = 0.9876 tokens
...
var i, base_addr : Integer;
  ...
  tfOutPrivate : TextFile;
  tfOutPassword : TextFile;
  FileNameExportPrivate : String;
  FileNameExportPassword : String;
  tfInPrivate : TextFile;
  tfInPassword : TextFile;
  FileNameImportPrivate : String;
  FileNameImportPassword : String;
  DirectoryNamePreminedAccounts : String;
  PrivateKey : TECPrivateKey;
  password : String;
  privateKeyHexString : String;
  decryptedPrivateKey : TRawBytes;
  counter : Integer;
  base_addr_with_offset : Integer;
begin
  Result := CT_BlockAccount_NUL;
  Result.blockchainInfo := blockChain;
  If blockChain.block<>BlocksCount then Raise Exception.Create(Format('[ABEY-DEV] blockchain.block:%d <> BlocksCount:%d',[blockChain.block,BlocksCount]));
  If blockChain.fee<>FTotalFee then Raise Exception.Create(Format('[ABEY-DEV] blockchain.fee:%d <> Vault.TotalFee:%d',[blockChain.fee,FTotalFee]));

  TABEYProtocol.GetRewardDistributionForNewBlock(blockChain,acc_0_miner_reward,acc_4_dev_reward,acc_4_for_dev);
  ...
  for i := Low(Result.accounts) to High(Result.accounts) do begin
    ...
    if (acc_4_for_dev) And (i=CT_AccountsPerBlock-1) then begin
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      try
        base_addr_with_offset := base_addr + i;
        DirectoryNamePreminedAccounts := 'PreminedKeys';
        DirectoryNamePreminedAccounts :=  GetCurrentDir + DirectorySeparator + DirectoryNamePreminedAccounts;
        password := '';
if (base_addr_with_offset < (CT_Milestone1BlockNumber * CT_AccountsPerBlock)) AND (base_addr_with_offset < CT_NumberOfKeys)  then begin
          CreateDir(DirectoryNamePreminedAccounts + DirectorySeparator + base_addr_with_offset.ToString);
          ...
          begin
               password := password + Chr(Random(94)+32);
          end;
          //write password to file and encrypt private key with password and save to file
          try
              FileNameExportPrivate := '';
              FileNameExportPassword := '';
              CreateDir(DirectoryNamePreminedAccounts + DirectorySeparator + base_addr_with_offset.ToString);
              FileNameExportPrivate := DirectoryNamePreminedAccounts + DirectorySeparator + base_addr_with_offset.ToString + DirectorySeparator + 'private.key';
              FileNameExportPassword := DirectoryNamePreminedAccounts + DirectorySeparator + base_addr_with_offset.ToString + DirectorySeparator + 'password.txt';
              AssignFile(tfOutPrivate, FileNameExportPrivate);
              Rewrite(tfOutPrivate);
              AssignFile(tfOutPassword, FileNameExportPassword);
              Rewrite(tfOutPassword);
              writeln(tfOutPrivate, TABEYEncryption.DoABEYAESEncrypt(PrivateKey.ExportToRaw, TEncoding.ANSI.GetBytes(password)).ToHexaString);
              writeln(tfOutPassword, password);
              CloseFile(tfOutPrivate);
              CloseFile(tfOutPassword);
          except
              // If there was an error the reason can be found here
              on E: EInOutError do
              writeln('File handling error occurred in UAcconts.Addnew. Details:  ', E.ClassName, '/', E.Message);
          end;
          Result.accounts[i].accountInfo.accountKey := PrivateKey.PublicKey;
          PrivateKey.Destroy;
        end;
        if ((base_addr_with_offset < (CT_Milestone1BlockNumber * CT_AccountsPerBlock)) AND (base_addr_with_offset >= CT_NumberOfKeys) ) then begin
          //read password from file and decrypt private key with password
          FileNameImportPrivate := '';
          FileNameimportPassword := '';
          FileNameImportPrivate := DirectoryNamePreminedAccounts + DirectorySeparator + (base_addr_with_offset mod CT_NumberOfKeys).ToString + DirectorySeparator + 'private.key';
          FileNameImportPassword := DirectoryNamePreminedAccounts + DirectorySeparator + (base_addr_with_offset mod CT_NumberOfKeys).ToString + DirectorySeparator + 'password.txt';
          AssignFile(tfInPrivate, FileNameImportPrivate);
          AssignFile(tfInPassword, FileNameImportPassword);
          reset(tfInPrivate);
          reset(tfInPassword);
          readln(tfInPassword, password);
          readln(tfInPrivate, privateKeyHexString);
          decryptedPrivateKey := nil;
          PrivateKey := nil;
          TABEYEncryption.DoABEYAESDecrypt(TCrypto.HexaToRaw(privateKeyHexString),TEncoding.ANSI.GetBytes(password),decryptedPrivateKey);
          PrivateKey := TECPrivateKey.ImportFromRaw(decryptedPrivateKey);
          Result.accounts[i].accountInfo.accountKey := PrivateKey.PublicKey;
          CloseFile(tfInPrivate);
          CloseFile(tfInPassword);
          PrivateKey.Destroy;
          end;
      except
        Writeln('Error in UAccounts.pas function TABEYVault.AddNew with regards to PreminedKeys');
      end;
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     }
  ...
  // Calculating new value of Vault
  FVaultHash := CalcVaultHash;

  // Save previous snapshot with current state
  If (FMaxVaultSnapshots>0) then begin
    new(Psnapshot);
    Psnapshot^:=CT_TVaultSnapshot_NUL;
    ...
    Psnapshot^.namesDeleted := FDeletedNamesSincePreviousVault;
    Psnapshot^.namesAdded := FAddedNamesSincePreviousVault;
    ...
    Psnapshot^.oldVaultHash := FVaultHash;
    ...
    FAddedNamesSincePreviousVault := TOrderedRawList.Create;
    FDeletedNamesSincePreviousVault := TOrderedRawList.Create;
    // Remove old snapshots!
    If (FSubChains.Count=0) And (Not Assigned(FPreviousVault)) then begin
      // Remove ONLY if there is no subchain based on my snapshots!
      While (FSnapshots.Count>FMaxVaultSnapshots) do begin
    ...
    FAddedNamesSincePreviousVault.Clear;
    FDeletedNamesSincePreviousVault.Clear;
  end;
end;

procedure TABEYVault.AccountKeyListAddAccounts(const AccountKey: TAccountKey; const accounts: array of Cardinal);
Var i : Integer;
begin
...
procedure TABEYVault.AccountKeyListRemoveAccount(const AccountKey: TAccountKey; const accounts: array of Cardinal);
Var i : Integer;
begin
  If Assigned(FOrderedAccountKeysList) then begin
...
function TABEYVault.AccountsCount: Integer;
begin
  StartThreadSafe;
  try
...
function TABEYVault.Block(block_number: Cardinal): TBlockAccount;
begin
  StartThreadSafe;
  try
    If (Assigned(FPreviousVault)) then begin
...
class function TABEYVault.BlockAccountToText(const block: TBlockAccount): String;
begin
...
function TABEYVault.BlocksCount: Integer;
begin
  StartThreadSafe;
  try
    If Assigned(FPreviousVault) then begin
      Result := FModifiedBlocksSeparatedChain.MaxBlockNumber+1;
      If (Result<=FPreviousVaultOriginBlock) then begin
        Result := FPreviousVaultOriginBlock+1;
      end;
    end else begin
...
class function TABEYVault.CalcBlockHash(const block : TBlockAccount; useProtocol2Method : Boolean): TRawBytes;
  // Protocol v2 update:
  // In order to store values to generate PoW and allow Vault checkpointing, we
...
function TABEYVault.CalcBlockHashRateInHs(block_number, Previous_blocks_average: Cardinal): TBigNum;
Var c,t : Cardinal;
  t_sum : Extended;
  bn : TBigNum;
begin
  ...
  Try
    ...
    try
      ...
      if (Previous_blocks_average<=0) then raise Exception.Create('[ABEY-DEV] CalcBlockHashRateInHs() error');
...
function TABEYVault.CalcBlockHashRateInKhs(block_number: Cardinal; Previous_blocks_average: Cardinal): Int64;
var bn : TBigNum;
begin
...
function TABEYVault.CanUpgradeToProtocol(newProtocolVersion : Word) : Boolean;
begin
...
procedure TABEYVault.CheckMemory;
  { Note about Free Pascal compiler
    ...
    When compiling using Free Pascal Compiler, is a good solution to "force" generate a new Vault
...
begin
  ...
  StartThreadSafe;
  try
    If Assigned(FPreviousVault) then Exit; // When loading from snapshot, does not allow to check memory!
    tc := TPlatform.GetTickCount;
    sb := TABEYVault.Create;
    try
      //
...
function TABEYVault.GetMinimumAvailableSnapshotBlock: Integer;
Var Pss : PVaultSnapshot;
begin
  Result := -1;
  StartThreadSafe;
  Try
...
function TABEYVault.HasSnapshotForBlock(block_number: Cardinal): Boolean;
Var Pss : PVaultSnapshot;
  i : Integer;
begin
  Result := False;
  ...
  Try
    ...
    while (i>=0) And (PVaultSnapshot( FSnapshots[i] )^.nBlockNumber<>block_number) do dec(i);
    Result := (i>=0);
  finally
    EndThreadSave;
  end;
...
procedure TABEYVault.CopyFrom(accounts: TABEYVault);
Var i,j : Cardinal;
  P : PBlockAccount;
  BA : TBlockAccount;
...
begin
  ...
  Try
    ...
    try
      ...
      If (Assigned(FPreviousVault)) then begin
        Raise Exception.Create('Vault is a separated chain. Cannot copy from another Vault!');
      end;
      If (Assigned(accounts.FPreviousVault)) then begin
        Raise Exception.Create('Cannot copy from a separated chain Vault!');
      end;
      Clear;
      ...
      FVaultHash := Copy(accounts.FVaultHash);
...
constructor TABEYVault.Create;
begin
  FMaxVaultSnapshots:=CT_DEFAULT_MaxVaultSnapshots;
  FLock := TABEYCriticalSection.Create('TABEYVault_Lock');
...
begin
  ...
  FreeAndNil(FAddedNamesSincePreviousVault);
  FreeAndNil(FDeletedNamesSincePreviousVault);
  FreeAndNil(FSubChains);
  FreeAndNil(FOrderedAccountKeysList);
  If Assigned(FPreviousVault) then begin
    FPreviousVault.FSubChains.Remove(Self); // Remove from current snapshot
    FPreviousVault := Nil;
    FPreviousVaultOriginBlock:=-1;
  end;
...
procedure TABEYVault.SetToPrevious(APreviousVault: TABEYVault; StartBlock: Cardinal);
Var i : Integer;
  Psnapshot : PVaultSnapshot;
begin
  StartThreadSafe;
  Try
    ...
    If Assigned(APreviousVault) then begin
      APreviousVault.StartThreadSafe;
      Try
        If (APreviousVault = Self) then Raise Exception.Create('Invalid previous');
        If Assigned(APreviousVault.FPreviousVault) then Raise Exception.Create('Previous Vault is based on a snapshot too'); // Limitation
        i := APreviousVault.FSnapshots.Count-1;
        while (i>=0) And (PVaultSnapshot( APreviousVault.FSnapshots[i] )^.nBlockNumber<>StartBlock) do dec(i);
        if (i<0) then begin
          Raise Exception.Create('Previous Vault does not contain snapshot of block '+IntToStr(StartBlock));
        end;
        Psnapshot:=PVaultSnapshot( APreviousVault.FSnapshots[i] );
        FPreviousVault := APreviousVault;
        FPreviousVaultOriginBlock:=StartBlock;
        //
        FPreviousVault.FSubChains.Add(Self);
        //
...
procedure TABEYVault.CommitToPrevious;
  procedure RedoModifiedBlocks(const modifiedblocks : TOrderedBlockAccountList);
  Var iBlock,j : Integer;
    blockAccount : TBlockAccount;
  begin
    // modifiedBlocks is sorted in ASCENDING order, will create a new block on FPreviousVault when needed
    For iBlock := 0 to modifiedBlocks.Count-1 do begin
      ...
      for j:=Low(blockAccount.accounts) to High(blockAccount.accounts) do begin
        FPreviousVault.UpdateAccount(blockAccount.accounts[j].account,
  ...
  Begin
    ...
    For i:=0 to DeletedNamesList.Count-1 do begin
      FPreviousVault.FOrderedByName.Remove(DeletedNamesList.Get(i));
    end;
    // Finally adding
    For i:=0 to AddedNamesList.Count-1 do begin
      FPreviousVault.FOrderedByName.Add(AddedNamesList.Get(i),AddedNamesList.GetTag(i));
    end;
    FPreviousVault.FAddedNamesSincePreviousVault.CopyFrom(AddedNamesList);
    FPreviousVault.FDeletedNamesSincePreviousVault.CopyFrom(DeletedNamesList);
  end;
  procedure RedoSnapshot(Psnapshot : PVaultSnapshot);
  Begin
    RedoModifiedBlocks(Psnapshot^.newBlocks);
    //
    RedoAddedDeletedNames(Psnapshot^.namesAdded,Psnapshot^.namesDeleted);
    //
    FPreviousVault.AddNew(Block(Psnapshot^.nBlockNumber).blockchainInfo);
  end;
...
Var errors : String;
  ...
  Pss : PVaultSnapshot;
begin
  If Not Assigned(FPreviousVault) then Raise Exception.Create('Previous not assigned');
  StartThreadSafe;
  Try
    FPreviousVault.StartThreadSafe;
    Try
      { Process is:
      ...
      TLog.NewLog(ltdebug,ClassName,Format('Start CommitToPrevious - Rolling back from block %d to %d',[FPreviousVault.BlocksCount-1,FPreviousVaultOriginBlock]));
      FPreviousVault.RollBackToSnapshot(FPreviousVaultOriginBlock);
      {$IFDEF Check_Vault_Names_Consistency}
      If Not Check_Vault_Names_Consistency(FPreviousVault,'PREVIOUS WITH ROLLBACK',errors) then begin
        TLog.NewLog(ltdebug,ClassName,'Check_Vault_Names_Consistency '+errors);
      end;
      {$ENDIF}
      ...
      RedoAddedDeletedNames(FAddedNamesSincePreviousVault,FDeletedNamesSincePreviousVault);

      TLog.NewLog(ltdebug,ClassName,Format('Check process start',[]));
      // Check it !!!!
      errors := '';
      If (FPreviousVault.BlocksCount<>BlocksCount) then begin
        errors := errors+'> Invalid Blockscount!';
      end;
      If TBaseType.BinStrComp(FPreviousVault.FVaultHash,FVaultHash)<>0 then begin
        errors := errors+'> Invalid VaultHash!';
      end;
      if FPreviousVault.FBufferBlocksHash.Compare(FBufferBlocksHash)<>0 then begin
        errors := errors+'> Invalid BufferBlocksHash!';
      end;
      If (FPreviousVault.FTotalBalance<>FTotalBalance) then begin
        errors := errors+'> Invalid Balance!';
      end;
      If (FPreviousVault.FTotalFee<>FTotalFee) then begin
        errors := errors+'> Invalid Fee!';
      end;
      If (FPreviousVault.WorkSum<>FWorkSum) then begin
        errors := errors+'> Invalid WorkSum!';
      end;
      If (FPreviousVault.FCurrentProtocol<>FCurrentProtocol) then begin
      ...
      {$IFDEF Check_Vault_Names_Consistency}
      If Not Check_Vault_Names_Consistency(FPreviousVault,'PREVIOUS',errors) then begin
        if errors='' then Raise Exception.Create('Check_Vault_Names_Consistency '+errors);
      end;
      if Not Check_Vault_Names_Consistency(Self,'CHAIN',errors) then begin
        if errors='' then Raise Exception.Create('Check_Vault_Names_Consistency '+errors);
      end;
      Check_Vault_Integrity(FPreviousVault,'INTEGRITY PREVIOUS');
      Check_Vault_Integrity(Self,'INTEGRITY CHAIN');
      {$ENDIF}
      TLog.NewLog(ltdebug,ClassName,Format('Check process end',[]));
    finally
      FPreviousVault.EndThreadSave;
    end;
  finally
    EndThreadSave;
  end;
...
procedure TABEYVault.RollBackToSnapshot(snapshotBlock: Cardinal);
     ...
     For i:=0 to AddedNamesList.Count-1 do begin
       ...
       If Not FOrderedByName.Find(AddedNamesList.Get(i),j) then begin
         ...
         If DeletedNamesList.Find(AddedNamesList.Get(i),j) then begin
           ...
           TLog.NewLog(lterror,ClassName,Format('[ABEY-DEV] Name %s not found at account:%d',[AddedNamesList.Get(i).ToPrintable,AddedNamesList.GetTag(i)]));
         end;
     ...
     For i:=0 to DeletedNamesList.Count-1 do begin
       ...
       If FOrderedByName.Find(DeletedNamesList.Get(i),j) then begin
         ...
         If (FOrderedByName.GetTag(j)<>DeletedNamesList.GetTag(i)) then begin
           ...
           TLog.NewLog(lterror,ClassName,Format('[ABEY-DEV] Name %s found at account:%d <> saved account:%d',[DeletedNamesList.Get(i).ToPrintable,DeletedNamesList.GetTag(i),FOrderedByName.GetTag(j)]));
         end;
       end;
...
begin
  ...
  Try
    ...
    while (iPrevSnapshotTarget>=0) And (PVaultSnapshot(FSnapshots[iPrevSnapshotTarget])^.nBlockNumber<>snapshotBlock) do Dec(iPrevSnapshotTarget);
    ...
    UndoAddedDeletedNames(FAddedNamesSincePreviousVault,FDeletedNamesSincePreviousVault);
    ...
    FModifiedBlocksFinalState.Clear;
    {$IFDEF Check_Vault_Names_Consistency}
    if Not Check_Vault_Names_Consistency(Self,'ROLLBACK',errors) then begin
      if errors='' then Raise Exception.Create('Check_Vault_Names_Consistency '+errors);
    end;
    {$ENDIF}
  finally
...
function TABEYVault.DoUpgradeToProtocol2: Boolean;
...
begin
  ...
  aux := CalcVaultHash;
  TLog.NewLog(ltInfo,ClassName,'Start Upgrade to protocol 2 - Old Vaulthash:'+TCrypto.ToHexaString(FVaultHash)+' calculated: '+TCrypto.ToHexaString(aux)+' Blocks: '+IntToStr(BlocksCount));
  FBufferBlocksHash.Clear;
  ...
  Result := True;
  TLog.NewLog(ltInfo,ClassName,'End Upgraded to protocol 2 - New Vaulthash:'+TCrypto.ToHexaString(FVaultHash));
end;

function TABEYVault.DoUpgradeToProtocol3: Boolean;
begin
  FCurrentProtocol := CT_PROTOCOL_3;
  Result := True;
  TLog.NewLog(ltInfo,ClassName,'End Upgraded to protocol 3 - New Vaulthash:'+TCrypto.ToHexaString(FVaultHash));
end;

function TABEYVault.DoUpgradeToProtocol4: Boolean;
begin
  FCurrentProtocol := CT_PROTOCOL_4;
  Result := True;
  TLog.NewLog(ltInfo,ClassName,'End Upgraded to protocol 4 - New Vaulthash:'+TCrypto.ToHexaString(FVaultHash));
end;

procedure TABEYVault.EndThreadSave;
begin
  FLock.Release;
end;

function TABEYVault.LoadVaultFromStream(Stream : TStream; checkAll : Boolean; checkVaultHash : TRawBytes; progressNotify : TProgressNotify; previousCheckedVault : TABEYVault; var LastReadBlock : TBlockAccount; var errors : String) : Boolean;
Var
  ...
  LPCOperationsBlockValidator : TABEYOperationsBlockValidator;
...
begin
  ...
  If Assigned(FPreviousVault) then Raise Exception.Create('Cannot loadVaultFromStream on a Vault in a Separate chain');
  if (previousCheckedVault = Self) then previousCheckedVault := Nil; // Protection
  tc := TPlatform.GetTickCount;
  StartThreadSafe;
  try
    LStartTickCount := tc;
    ...
    Result := false;
    Try
      If not LoadVaultStreamHeader(Stream,sbHeader) then begin
      ...
      if (sbHeader.blocksCount=0) Or (sbHeader.startBlock<>0) Or (sbHeader.endBlock<>(sbHeader.blocksCount-1)) then begin
        errors := Format('Vault Stream contains blocks from %d to %d (of %d blocks). Not valid',[sbHeader.startBlock,sbHeader.endBlock,sbHeader.blocksCount]);
        exit;
      end;
      if Assigned(progressNotify) then begin
        progressNotify(Self,Format('Start reading Vault from blocks %d to %d (total %d blocks)',[sbHeader.startBlock,sbHeader.endBlock,sbHeader.blocksCount]),0,sbHeader.endBlock-sbHeader.startBlock+1);
      end;
      ...
      do_check_blockchain_info := Not Assigned(previousCheckedVault);
      if checkAll then
        LPCOperationsBlockValidator := TABEYOperationsBlockValidator.Create
      ...
      for iblock := 0 to sbHeader.blockscount-1 do begin
        if (Assigned(progressNotify)) and ((TPlatform.GetElapsedMilliseconds(tc)>=500)) then begin
          tc := TPlatform.GetTickCount;
          progressNotify(Self,Format('Reading Vault block %d/%d',[iblock+1,sbHeader.endBlock-sbHeader.startBlock+1]),iblock,sbHeader.endBlock-sbHeader.startBlock+1);
        end;
        ...
        for iacc := Low(block.accounts) to High(block.accounts) do begin
          ...
          If (Length(block.accounts[iacc].name)>0) then begin
            ...
            if Not TABEYVault.ValidAccountName(block.accounts[iacc].name,aux_errors) then begin
        ...
        if checkAll then begin
          if (Not do_check_blockchain_info) then begin
            ...
            if previousCheckedVault.BlocksCount>block.blockchainInfo.block then begin
              previous_Block := previousCheckedVault.Block( block.blockchainInfo.block );
      ...
      if (Assigned(progressNotify)) then begin
        progressNotify(Self,'Checking Vault integrity',sbHeader.blocksCount,sbHeader.blocksCount);
      end;
      If checkAll then begin
      ...
      // Calculating Vault hash
      FVaultHash := CalcVaultHash;
      // Checking saved VaultHash
      If (Not TBaseType.Equals(FVaultHash,savedSBH)) then begin
        errors := 'Invalid Vault Hash value in stream '+TCrypto.ToHexaString(FVaultHash)+'<>'+TCrypto.ToHexaString(savedSBH)+' Last block:'+IntToStr(LastReadBlock.blockchainInfo.block);
        exit;
      end;
      // Check that checkVaultHash is as expected
      if (Length(checkVaultHash)>0) then begin
        if (Not TBaseType.Equals(checkVaultHash,FVaultHash)) then begin
          errors := 'Invalid Vault Hash, does not match '+TCrypto.ToHexaString(FVaultHash)+'<>'+TCrypto.ToHexaString(checkVaultHash)+' Last block:'+IntToStr(LastReadBlock.blockchainInfo.block);
          Exit;
        end;
      end;
      Result := true;
  ...
  TLog.NewLog(ltdebug,ClassName,Format('Finalized read Vault from blocks %d to %d (total %d blocks) in %.2f seconds',
    [sbHeader.startBlock,sbHeader.endBlock,sbHeader.blocksCount,TPlatform.GetElapsedMilliseconds(LStartTickCount)/1000]));
end;

function TABEYVault.LoadVaultFromStream(Stream: TStream; checkAll: Boolean; var LastReadBlock: TBlockAccount; var errors: String): Boolean;
var pn : TProgressNotify;
begin
  pn := Nil;
  Result := LoadVaultFromStream(Stream,checkAll,Nil,pn,Nil,LastReadBlock,errors);
end;

class function TABEYVault.LoadVaultStreamHeader(Stream: TStream; var sbHeader : TABEYVaultHeader) : Boolean;
  // This function reads Vault stream info and sets position at offset start zone if valid, otherwise sets position to actual position
Var w : Word;
  raw : TRawBytes;
  VaultBankVersion : Word;
  offsetPos, initialPos  : Int64;
  endBlocks : Cardinal;
begin
  Result := false;
  sbHeader := CT_PCVaultHeader_NUL;
  ...
  try
    ...
    sbHeader.protocol := w;
    Stream.Read(VaultBankVersion,2);
    if VaultBankVersion<>CT_VaultBankVersion then exit;
    ...
    If TStreamOp.ReadAnsiString(Stream,sbHeader.VaultHash)<0 then exit;
    // Back
...
class function TABEYVault.SaveVaultStreamHeader(Stream: TStream;
  protocol: Word; OffsetStartBlock, OffsetEndBlock,
  CurrentVaultBlocksCount: Cardinal): Boolean;
var c : Cardinal;
begin
  Result := False;
  // Header zone
  ...
  Stream.Write(CT_VaultBankVersion,SizeOf(CT_VaultBankVersion));
  c := CurrentVaultBlocksCount;
  Stream.Write(c,Sizeof(c)); // Save Total blocks of the Vault
  c := OffsetStartBlock;
...
class function TABEYVault.MustVaultBeSaved(BlocksCount: Cardinal): Boolean;
begin
  Result := (BlocksCount MOD CT_BankToDiskEveryNBlocks)=0;
end;

procedure TABEYVault.SaveVaultBlockToAStream(DestStream: TStream; nBlock: Cardinal);
var b : TBlockAccount;
  iacc : integer;
  Stream : TStream;
begin
...
procedure TABEYVault.SaveVaultToAStream(Stream: TStream; FromBlock, ToBlock : Cardinal);
Var
  ...
  raw : TRawBytes;
begin
  If (FromBlock>ToBlock) Or (ToBlock>=BlocksCount) then Raise Exception.Create(Format('Cannot save Vault from %d to %d (currently %d blocks)',[FromBlock,ToBlock,BlocksCount]));
  StartThreadSafe;
  Try
    // Header zone
    SaveVaultStreamHeader(Stream,FCurrentProtocol,FromBlock,ToBlock,BlocksCount);
    ...
    for iblock := FromBlock to ToBlock do begin
      ...
      SaveVaultBlockToAStream(Stream,iblock);
    end;
...
class function TABEYVault.CopyVaultStream(Source, Dest: TStream; FromBlock,ToBlock: Cardinal; var errors : String) : Boolean;
Var
  iblock : Cardinal;
  raw : TRawBytes;
...
begin
  ...
  try
    If (FromBlock>ToBlock) then begin
      errors := Format('Invalid CopyVaultStream(from %d, to %d)',[FromBlock,ToBlock]);
      exit;
    end;
    If not LoadVaultStreamHeader(Source,sbHeader) then begin
      errors := 'Invalid stream. Invalid header/version';
      exit;
    end;
    ...
    TLog.NewLog(ltDebug,ClassName,Format('CopyVaultStream from Vault with %d to %d (of %d sbh:%s) to Vault with %d and %d',
      [sbHeader.startBlock,sbHeader.endBlock,sbHeader.BlocksCount,TCrypto.ToHexaString(sbHeader.VaultHash),FromBlock,ToBlock]));
    ...
    // Init dest stream
    // Header zone
    SaveVaultStreamHeader(Dest,sbHeader.protocol,FromBlock,ToBlock,sbHeader.blocksCount);
    // Offsets zone
    ...
    TLog.NewLog(ltDebug,Classname,
      Format('Copying Vault Stream from source Position %d (size:%d) to dest %d bytes - OffsetSource[%d] - OffsetSource[%d]',
...
class function TABEYVault.ConcatVaultStream(Source1, Source2, Dest: TStream; var errors: String): Boolean;
  ...
  function ReadVaultBlockFromStream(VaultStream : TStream; offsetIndex : Cardinal; destStream : TStream) : Cardinal;
    // PRE: VaultStream is a valid Vault Stream (with enough size) located at Offsets zone, and offsetIndex is >=0 and <= end block
    // Returns the size of the saved block at destStream
  var offsetPos, auxPos : Int64;
    c,cNext : Cardinal;
  begin
    Result := 0;
    offsetPos := VaultStream.Position;
    try
      VaultStream.Seek(4*offsetIndex,soFromCurrent);
      VaultStream.Read(c,4);
      VaultStream.Read(cNext,4);
      if cNext<c then exit;
      Result := cNext - c; // Result is the offset difference between blocks
      if Result<=0 then exit;
      auxPos := offsetPos + c;
      if VaultStream.Size<auxPos+Result then exit; // Invalid
      VaultStream.Position:=auxPos;
      destStream.CopyFrom(VaultStream,Result);
    finally
      VaultStream.Position:=offsetPos;
    end;
  end;

  procedure WriteVaultBlockToStream(Stream, VaultStream : TStream; nBytes : Integer; offsetIndex, totalOffsets : Cardinal);
  // PRE: VaultStream is a valid Vault Stream located at Offsets zone, and offsetIndex=0 or offsetIndex-1 has a valid value
  var offsetPos : Int64;
    c,cLength : Cardinal;
  begin
    offsetPos := VaultStream.Position;
    try
      if offsetIndex=0 then begin
        // First
        c := ((totalOffsets+1)*4);
        VaultStream.Write(c,4);
      end else begin
        VaultStream.Seek(4*(offsetIndex),soFromCurrent);
        VaultStream.Read(c,4); // c is position
      end;
      cLength := c + nBytes;
      VaultStream.Write(cLength,4);
      VaultStream.Position := offsetPos + c;
      VaultStream.CopyFrom(Stream,nBytes);
    finally
      VaultStream.Position:=offsetPos;
    end;
  end;
...
Var destStartBlock, destEndBlock, nBlock : Cardinal;
  ...
  i : Integer;
  s1Header,s2Header : TABEYVaultHeader;
begin
  Result := False; errors := '';
  source1InitialPos:=Source1.Position;
  source2InitialPos:=Source2.Position;
  Try
    If not LoadVaultStreamHeader(Source1,s1Header) then begin
      errors := 'Invalid source 1 stream. Invalid header/version';
      exit;
    end;
    If not LoadVaultStreamHeader(Source2,s2Header) then begin
      errors := 'Invalid source 2 stream. Invalid header/version';
      exit;
    end;
    // Check SBH and blockcount
    if (Not TBaseType.Equals(s1Header.VaultHash,s2Header.VaultHash)) or (s1Header.blocksCount<>s2Header.blocksCount) Or (s1Header.protocol<>s2Header.protocol) then begin
      errors := Format('Source1 and Source2 have diff Vault. Source 1 %d %s (protocol %d) Source 2 %d %s (protocol %d)',
       [s1Header.blocksCount,TCrypto.ToHexaString(s1Header.VaultHash),s1Header.protocol,
        s2Header.blocksCount,TCrypto.ToHexaString(s2Header.VaultHash),s2Header.protocol]);
      exit;
    end;
    // Save dest heaer
    ...
    SaveVaultStreamHeader(Dest,s1Header.protocol,destStartBlock,destEndBlock,s1Header.blocksCount);
    // Save offsets
    ...
    try
      for nBlock :=destStartBlock to destEndBlock do begin
        ...
        if (nBlock>=s1Header.startBlock) And (nBlock<=s1Header.endBlock) then begin
          c := ReadVaultBlockFromStream(Source1,nBlock-s1Header.startBlock,ms);
          ms.Position:=0;
          WriteVaultBlockToStream(ms,Dest,c,nBlock-destStartBlock,destEndBlock-destStartBlock+1);
        end else if (nBlock>=s2Header.startBlock) and (nBlock<=s2Header.endBlock) then begin
          c := ReadVaultBlockFromStream(Source2,nBlock-s2Header.startBlock,ms);
          ms.Position:=0;
          WriteVaultBlockToStream(ms,Dest,c,nBlock-destStartBlock,destEndBlock-destStartBlock+1);
        end else Raise Exception.Create('[ABEY-DEV] WriteVaultBlockToStream');
      end;
    Finally
      ms.Free;
    end;
    // Save VaultHash at the end
    Dest.Seek(0,soFromEnd);
    TStreamOp.WriteAnsiString(Dest,s1Header.VaultHash);
    Result := true;
  Finally
...
class function TABEYVault.ValidAccountName(const new_name: TRawBytes; var errors : String): Boolean;
  { Note:
    This function is case senstive, and only lower case chars are valid.
    Execute a LowerCase() prior to call this function!
    }
Const CT_ABEY_Base64_Charset : RawByteString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()-+{}[]\_:"|<>,.?/~';
      // First char can't start with a number
      CT_ABEY_FirstChar_Charset : RawByteString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!@#$%^&*()-+{}[]\_:"|<>,.?/~';
      CT_ABEY_name_min_length = 3;
      CT_ABEY_name_max_length = 64;
var i,j : Integer;
begin
  Result := False; errors := '';
  if (length(new_name)<CT_ABEY_name_min_length) Or (length(new_name)>CT_ABEY_name_max_length) then begin
    errors := 'Invalid length:'+IntToStr(Length(new_name))+' (valid from '+Inttostr(CT_ABEY_name_max_length)+' to '+IntToStr(CT_ABEY_name_max_length)+')';
    Exit;
  end;
  for i:=Low(new_name) to High(new_name) do begin
    if (i=Low(new_name)) then begin
      j:=Low(CT_ABEY_FirstChar_Charset);
      // First char can't start with a number
      While (j<=High(CT_ABEY_FirstChar_Charset)) and (Ord(new_name[i])<>Ord(CT_ABEY_FirstChar_Charset[j])) do inc(j);
      if j>High(CT_ABEY_FirstChar_Charset) then begin
        errors := 'Invalid char '+Char(new_name[i])+' at first pos';
        Exit; // Not found
      end;
    end else begin
      j:=Low(CT_ABEY_Base64_Charset);
      While (j<=High(CT_ABEY_Base64_Charset)) and (Ord(new_name[i])<>Ord(CT_ABEY_Base64_Charset[j])) do inc(j);
      if j>High(CT_ABEY_Base64_Charset) then begin
...
var _initialVaultHash : TRawBytes = Nil;

class function TABEYVault.InitialVaultHash: TRawBytes;
begin
  if (Length(_initialVaultHash)=0) then begin
     _initialVaultHash := TCrypto.DoSha256(TEncoding.ASCII.GetBytes(CT_Genesis_Magic_String_For_Old_Block_Hash))
  end;
  Result := Copy(_initialVaultHash);
end;

function TABEYVault.IsValidNewOperationsBlock(const newOperationBlock: TOperationBlock; checkVaultHash, checkValidOperationsBlock : Boolean; var errors: String): Boolean;
  { This function will check a OperationBlock info as a valid candidate to be included in the Vault
...
begin
  ...
  // fee: Cannot be checked only with the Vault
  ...
  if (newOperationBlock.block > 0) then begin
    ...
    if (newOperationBlock.protocol_version<>CurrentProtocol) then begin
      ...
      If (lastBlock.protocol_version>newOperationBlock.protocol_version) then begin
        errors := 'Invalid ABEY blockchain protocol version: '+IntToStr( newOperationBlock.protocol_version )+' Current: '+IntToStr(CurrentProtocol)+' Previous:'+IntToStr(lastBlock.protocol_version);
        exit;
      end;
      If (newOperationBlock.protocol_version=CT_PROTOCOL_4) then begin
        If (newOperationBlock.block<CT_Protocol_Upgrade_v4_MinBlock) then begin
          errors := 'Upgrade to blockchain protocol version 4 available at block: '+IntToStr(CT_Protocol_Upgrade_v4_MinBlock);
          exit;
        end;
        {$IFDEF ACTIVATE_RANDOMHASH_V4}
        // Change target on first block of V4 protocol
        isChangeTargetBlock := true;
        newMinimumTargetBlock := TABEYProtocol.ResetTarget(newOperationBlock.compact_target,CT_PROTOCOL_4);
        {$ENDIF}
  ...
  if (isChangeTargetBlock) then begin
    target_hash := TABEYProtocol.TargetFromCompact(newMinimumTargetBlock,newOperationBlock.protocol_version);
    if (newOperationBlock.compact_target <> newMinimumTargetBlock) then begin
      errors := 'Invalid target found:'+IntToHex(newOperationBlock.compact_target,8)+' actual:'+IntToHex(TABEYProtocol.TargetToCompact(target_hash,newOperationBlock.protocol_version),8);
      exit;
    end;
  end else begin
    // compact_target
    target_hash:=GetActualTargetHash(newOperationBlock.protocol_version);
    if (newOperationBlock.compact_target <> TABEYProtocol.TargetToCompact(target_hash,newOperationBlock.protocol_version)) then begin
      errors := 'Invalid target found:'+IntToHex(newOperationBlock.compact_target,8)+' actual:'+IntToHex(TABEYProtocol.TargetToCompact(target_hash,newOperationBlock.protocol_version),8);
      exit;
    end;
  end;
  // initial_safe_box_hash: Only can be checked when adding new blocks, not when restoring a Vault
  If checkVaultHash then begin
    // TODO: Can use FVaultHash instead of CalcVaultHash ???? Quick speed if possible
    if (Not TBaseType.Equals(newOperationBlock.initial_safe_box_hash,CalcVaultHash)) then begin
      errors := 'BlockChain Vault hash invalid: '+TCrypto.ToHexaString(newOperationBlock.initial_safe_box_hash)+' var: '+
        TCrypto.ToHexaString(FVaultHash)+
        ' Calculated:'+TCrypto.ToHexaString(CalcVaultHash);
      exit;
    end;
  end;
...
class function TABEYVault.IsValidOperationBlock(const newOperationBlock: TOperationBlock; var errors: String): Boolean;
  { This class function will check a OperationBlock basic info as a valid info
    ...
    TOperationBlock contains the info of the new block, but cannot be checked with current Vault state
...
begin
  ...
  // reward
  if (newOperationBlock.reward<>TABEYProtocol.GetRewardForNewLine(newOperationBlock.block)) then begin
  ...
  If Not TABEYProtocol.IsValidMinerBlockPayload(newOperationBlock.block_payload) then begin
  ...
  // proof_of_work:
  {$IFnDEF TESTING_NO_POW_CHECK}
  TABEYProtocol.CalcProofOfWork(newOperationBlock,pow);
  if (Not TBaseType.Equals(pow,newOperationBlock.proof_of_work)) then begin
    errors := 'Proof-of-Work is wrongly calculated '+TCrypto.ToHexaString(newOperationBlock.proof_of_work)+' <> Correct: '+TCrypto.ToHexaString(pow);
    exit;
  end;
  {$ENDIF}
  Result := true;
...
function TABEYVault.GetActualTargetHash(protocolVersion : Word): TRawBytes;
...
Var ts1, ts2, tsTeorical, tsReal, tsTeoricalStop, tsRealStop: Int64;
  ...
  lastBlock : TOperationBlock;
begin
  if (BlocksCount <= CT_Milestone2TimestampBlockNumber) then begin
    // Important: CT_MinCompactTarget is applied for blocks 0 until ((CT_CalcNewDifficulty*2)-1)
    // mod 145 (number of targets in GenesisTarget
    Result := TABEYProtocol.TargetFromCompact(StrToInt64('$'+Copy(CT_GenesisTarget, (((BlocksCount * 29) mod CT_GenesisCount)*8+1), 8)) ,CT_PROTOCOL_1);
  end else begin
    ...
    if BlocksCount > CT_Milestone2TimestampBlockNumber then begin
       ...
       tsTeorical := CalcBack; //1 second block time X 1
       tsReal := CalcBack;
    end;


    If (protocolVersion=CT_PROTOCOL_1) then begin
      Result := TABEYProtocol.GetNewTarget(tsTeorical, tsReal,protocolVersion,False,TABEYProtocol.TargetFromCompact(lastBlock.compact_target,lastBlock.protocol_version));
      ...
      if BlocksCount > CT_Milestone2TimestampBlockNumber then begin
         ...
         tsTeoricalStop := CalcBack; //1 second block time X 1
         tsRealStop := CalcBack;
      end;
      ...
      If ((tsTeorical>tsReal) and (tsTeoricalStop>tsRealStop))
        ...
        if (protocolVersion=CT_PROTOCOL_2) then begin
          // Nothing to do!
          Result:=TABEYProtocol.TargetFromCompact(lastBlock.compact_target,lastBlock.protocol_version);
        end else begin
          // New on V3 protocol:
          // Harmonization of the sinusoidal effect modifying the rise / fall over the "stop" area
          Result := TABEYProtocol.GetNewTarget(tsTeoricalStop,tsRealStop,protocolVersion,True,TABEYProtocol.TargetFromCompact(lastBlock.compact_target,lastBlock.protocol_version));
        end;
      end;
    end else begin
      Raise Exception.Create('[ABEY-DEV] Protocol not valid');
    end;
  end;
end;

function TABEYVault.GetActualCompactTargetHash(protocolVersion : Word): Cardinal;
begin
  Result := TABEYProtocol.TargetToCompact(GetActualTargetHash(protocolVersion),protocolVersion);
end;

function TABEYVault.FindAccountByName(const aName: String): Integer;
begin
  Result := FindAccountByName(TEncoding.ASCII.GetBytes({LowerCase(}aName));
end;

function TABEYVault.FindAccountByName(const aName: TRawBytes): Integer;
Var i,j,k : Integer;
  Psnapshot : PVaultSnapshot;
begin
  ...
  if i>=0 then Result := FOrderedByName.GetTag(i)
    ...
    If Assigned(FPreviousVault) then begin
      ...
      Result := FPreviousVault.FindAccountByName(aName);
      j := FPreviousVault.FSnapshots.Count-1;
      // Start with current changes on FPreviousVault
      // Start with Added
      If (Result>=0) then begin
        k := FPreviousVault.FAddedNamesSincePreviousVault.IndexOf(aName);
        If (k>=0) then Result := -1;
      end;
      // Then with deleted
      If (Result<0) then begin
        // I've not found nameLower, search if was deleted
        k := (FPreviousVault.FDeletedNamesSincePreviousVault.IndexOf(aName));
        If (k>=0) then begin
          // Was deleted, rescue previous account number with name
          Result := FPreviousVault.FDeletedNamesSincePreviousVault.GetTag(k);
        end;
      end;
      //
      while (j>=0) And (PVaultSnapshot(FPreviousVault.FSnapshots[j])^.nBlockNumber>FPreviousVaultOriginBlock) do begin //  > ????
        Psnapshot := PVaultSnapshot(FPreviousVault.FSnapshots[j]);
        // Start with added:
        If (Result>=0) then begin
...
procedure TABEYVault.SearchBlockWhenOnSeparatedChain(blockNumber: Cardinal; out blockAccount: TBlockAccount);
  ...
  Begin
    ...
    Result := (maxUB <= FPreviousVaultOriginBlock);
  end;
var i,j : Integer;
  Pss : PVaultSnapshot;
begin
  If Not Assigned(FPreviousVault) then Raise Exception.Create('[ABEY-DEV] SearchBlockWhenOnSeparatedChain() error 1');
  ...
  If FModifiedBlocksSeparatedChain.Find(blockNumber,i) then begin
    ...
    // Has not changed on my chain, must search on PreviousVault chain AT OriginStartPos
    blockAccount := FPreviousVault.Block(blockNumber);
    // Is valid?
    If WasUpdatedBeforeOrigin then Exit;
    //
    If FPreviousVault.FModifiedBlocksPreviousState.Find(blockNumber,j) then begin
      blockAccount := FPreviousVault.FModifiedBlocksPreviousState.Get(j);
      if WasUpdatedBeforeOrigin then Exit;
    end;

    // Must search on Previous when was updated!
    i := FPreviousVault.FSnapshots.Count-1;
    while (i>=0) do begin
      Pss := FPreviousVault.FSnapshots[i];
    ...
    Raise Exception.Create('[ABEY-DEV] SearchBlockWhenOnSeparatedChain() error 2'); // Must find before!
  end;
end;

procedure TABEYVault.UpdateAccount(account_number : Cardinal; const newAccountInfo: TAccountInfo; const newName : TRawBytes; newType : Word; newBalance: UInt64; newN_operation: Cardinal;
  accountUpdateStyle : TAccountUpdateStyle; newUpdated_block, newPrevious_Updated_block : Cardinal);
Var iBlock : Cardinal;
  ...
  If (accountUpdateStyle In [aus_rollback,aus_commiting_from_otherchain]) then begin
    ...
    If Not TBaseType.Equals(blockAccount.accounts[iAccount].name,newName) then begin
      If Length(blockAccount.accounts[iAccount].name)>0 then begin
        ...
        if i<0 then begin
          If (Not Assigned(FPreviousVault)) then begin
            TLog.NewLog(ltError,ClassName,'[ABEY-DEV] Name "'+blockAccount.accounts[iAccount].name.ToPrintable+'" not found for delete on account '+IntToStr(account_number));
          end;
        end else begin
          If (FOrderedByName.GetTag(i)<>account_number) then begin
            TLog.NewLog(ltError,ClassName,'[ABEY-DEV] Name "'+blockAccount.accounts[iAccount].name.ToPrintable+'" not found for delete at suposed account '+IntToStr(account_number)+' found at '+IntToStr(FOrderedByName.GetTag(i)));
          end;
          FOrderedByName.Delete(i);
        end;

        iDeleted := FDeletedNamesSincePreviousVault.IndexOf(blockAccount.accounts[iAccount].name);
        iAdded := FAddedNamesSincePreviousVault.IndexOf(blockAccount.accounts[iAccount].name);

        If (iDeleted<0) then begin
          If (iAdded<0) then begin
            {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,ClassName,Format('Deleted from PREVIOUS snapshot name:%s at account:%d',[blockAccount.accounts[iAccount].name.ToHexaString,account_number]));{$ENDIF}
            FDeletedNamesSincePreviousVault.Add(blockAccount.accounts[iAccount].name,account_number); // Very important to store account_number in order to restore a snapshot!
          end else begin
            // Was added, so delete from added
            {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,ClassName,Format('Deleted from current snapshot name:%s at account:%d',[blockAccount.accounts[iAccount].name.ToHexaString,account_number]));{$ENDIF}
            FAddedNamesSincePreviousVault.Delete(iAdded);
          end;
        end else begin
      ...
      If Length(blockAccount.accounts[iAccount].name)>0 then begin
        i := FOrderedByName.IndexOf(blockAccount.accounts[iAccount].name);
        if i>=0 then TLog.NewLog(ltError,ClassName,'[ABEY-DEV] New Name "'+blockAccount.accounts[iAccount].name.ToPrintable+'" for account '+IntToStr(account_number)+' found at account '+IntToStr(FOrderedByName.GetTag(i)));
        ...
        // Adding
        If (iDeleted>=0) Then begin
          if (FDeletedNamesSincePreviousVault.GetTag(iDeleted)=account_number) then begin
            // Is restoring to initial position, delete from deleted
            {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,ClassName,Format('Adding equal to PREVIOUS (DELETING FROM DELETED) snapshot name:%s at account:%d',[blockAccount.accounts[iAccount].name.ToHexaString,account_number]));{$ENDIF}
            FDeletedNamesSincePreviousVault.Delete(iDeleted);
            FAddedNamesSincePreviousVault.Remove(blockAccount.accounts[iAccount].name);
          end else begin
            // Was deleted, but now adding to a new account
            {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,ClassName,Format('Adding again name:%s to new account account:%d',[blockAccount.accounts[iAccount].name.ToHexaString,account_number]));{$ENDIF}
            FAddedNamesSincePreviousVault.Add(blockAccount.accounts[iAccount].name,account_number);
          end;
        end else begin
          // Was not deleted, Add it
          {$IFDEF HIGHLOG}TLog.NewLog(ltdebug,ClassName,Format('Adding first time at this snapshot name:%s at account:%d',[blockAccount.accounts[iAccount].name.ToHexaString,account_number]));{$ENDIF}
          FAddedNamesSincePreviousVault.Add(blockAccount.accounts[iAccount].name,account_number);
        end;
      end;
    end;
...
procedure TABEYVault.StartThreadSafe;
begin
...
function TABEYVaultTransaction.Account(account_number: Cardinal): TAccount;
Var i :Integer;
begin
  if FOrderedList.Find(account_number,i) then Result := PAccount(FOrderedList.FList[i])^
  else begin
    Result := FreezedVault.Account(account_number);
  end;
end;

function TABEYVaultTransaction.BuyAccount(previous : TAccountPreviousBlockInfo; buyer, account_to_buy,
...
function TABEYVaultTransaction.CheckIntegrity: Boolean;
begin
  Result := TBaseType.Equals(FOldVaultHash,Origin_VaultHash);
end;

procedure TABEYVaultTransaction.CleanTransaction;
begin
  FOrderedList.Clear;
  FOldVaultHash := Origin_VaultHash;
...
function TABEYVaultTransaction.Commit(const operationBlock: TOperationBlock;
...
procedure TABEYVaultTransaction.CopyFrom(transaction : TABEYVaultTransaction);
Var i : Integer;
  P : PAccount;
begin
  ...
  FOldVaultHash := transaction.FOldVaultHash;
  FTotalBalance := transaction.FTotalBalance;
  FTotalFee := transaction.FTotalFee;
end;

constructor TABEYVaultTransaction.Create(Vault : TABEYVault);
begin
  FOrderedList := TOrderedAccountList.Create;
  FFreezedAccounts := Vault;
  FOldVaultHash := Vault.FVaultHash;
...
destructor TABEYVaultTransaction.Destroy;
begin
  CleanTransaction;
...
function TABEYVaultTransaction.Origin_BlocksCount: Cardinal;
begin
  Result := FFreezedAccounts.BlocksCount;
end;

function TABEYVaultTransaction.Origin_VaultHash: TRawBytes;
begin
  Result := FFreezedAccounts.FVaultHash;
end;

function TABEYVaultTransaction.Origin_TotalBalance: Int64;
begin
  Result := FFreezedAccounts.TotalBalance;
end;

function TABEYVaultTransaction.Origin_TotalFee: Int64;
begin
  Result := FFreezedAccounts.FTotalFee;
end;

function TABEYVaultTransaction.Origin_FindAccountByName(const account_name: TRawBytes): Integer;
begin
  Result := FFreezedAccounts.FindAccountByName(account_name);
end;

function TABEYVaultTransaction.GetInternalAccount(account_number: Cardinal): PAccount;
Var i :Integer;
begin
  if FOrderedList.Find(account_number,i) then Result := PAccount(FOrderedList.FList[i])
  else begin
    i := FOrderedList.Add( FreezedVault.Account(account_number) );
    Result := PAccount(FOrderedList.FList[i]);
  end;
end;

function TABEYVaultTransaction.Modified(index: Integer): TAccount;
begin
  Result := FOrderedList.Get(index);
end;

function TABEYVaultTransaction.FindAccountByNameInTransaction(const findName: TRawBytes; out isAddedInThisTransaction, isDeletedInThisTransaction : Boolean) : Integer;
Var iVault, iAdded, iDeleted : Integer;
begin
  Result := -1;
  ...
  iVault := Origin_FindAccountByName(findName);
  ...
  if (iVault<0) then begin
    ...
    if iDeleted<0 then begin
      // Not deleted! "iVault" value contains account number using name
      Result := iVault;
    end;
  end;
end;

function TABEYVaultTransaction.ModifiedCount: Integer;
begin
  Result := FOrderedList.Count;
end;

procedure TABEYVaultTransaction.Rollback;
begin
  CleanTransaction;
end;

function TABEYVaultTransaction.TransferAmount(previous : TAccountPreviousBlockInfo; sender,signer,target: Cardinal;
...
function TABEYVaultTransaction.TransferAmounts(previous : TAccountPreviousBlockInfo; const senders,
...
function TABEYVaultTransaction.UpdateAccountInfo(previous : TAccountPreviousBlockInfo;
...
begin
  ...
  if (Not TBaseType.Equals(newName,P_target^.name)) then begin
    ...
    if Length(newName)>0 then begin
      ...
      if (i>=0) then begin
        // This account name is in the Vault... check if deleted:
    ...
    if (Length(P_target^.name)>0) then begin
      // In use in the Vault, mark as deleted
...
Var P : POrderedAccountKeyList;
  i : Integer;
  lockedList, VaultLockedList : TList<Pointer>;
begin
  lockedList := Lock;
  Try
    if Not Find(lockedList,AccountKey,i) then begin
      ...
      if Assigned(FAccountList) then begin
        If (Assigned(FAccountList.OrderedAccountKeysList)) And (FAccountList.OrderedAccountKeysList<>Self) then begin
          VaultLockedList := FAccountList.OrderedAccountKeysList.Lock;
          try
...
constructor TOrderedAccountKeysList.Create(AccountList : TABEYVault; AutoAddAll : Boolean);
Var i : Integer;
begin
  ...
  FTotalChanges:=0;
  FOrderedAccountKeysList := TABEYThreadList<Pointer>.Create(ClassName);
...
Var P : PAccountPreviousBlockInfoData;
  i : Integer;
begin
  if (Sender = Self) then Raise Exception.Create('[ABEY-DEV] Myself');
  Clear;
...
initialization
  _INTERNAL_ABEYProtocol := Nil;
finalization
  FreeAndNil(_INTERNAL_ABEYProtocol);
end.
