...
type
  ...
  private
    ...
    FRemoteHost : String;
    FRemotePort : Word;
    FSendBufferLock : TABEYCriticalSection;
    {$ENDIF}
  ...
  TBufferedNetTcpIpClientThread = Class(TABEYThread)
  ...
  TTcpIpSocketThread = Class(TABEYThread)
  private
  ...
  TTcpIpServerListenerThread = Class(TABEYThread)
  private
  ...
  private
    ...
    FActive : Boolean;
    {$ENDIF}
    FNetClients : TABEYThreadList<TNetTcpIpClient>;
...
begin
  ...
  {$ENDIF}
  FLock := TABEYCriticalSection.Create('TNetTcpIpClient_Lock');
  {$IFDEF Synapse}
  FSendBufferLock := TABEYCriticalSection.Create('TNetTcpIpClient_SendBufferLock');
...
begin
  ...
  FCritical := TABEYCriticalSection.Create('TBufferedNetTcpIpClient_Critical');
...
begin
  ...
  FActive := false;
  {$ENDIF}
  FNetClients := TABEYThreadList<TNetTcpIpClient>.Create('TNetTcpIpServer_NetClients');
end;
