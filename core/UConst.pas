...
Const
  ...
  CT_MaxNumberOfActiveBlocks = 262800; // one year
  CT_MaxNumberOfPassiveBlocks = 262800;  //other contract parameters will use CT_MaxCardinalValue
  CT_MaxNumberOfTempFiles = 10000; // files will be deleted after this


  CT_NumberOfKeys : Cardinal = 15;
  ...
  CT_TargetLockOfAccountForContract = 4294967295; // max cardinal value aprox 16,000 years given 2 minute blocktime
  ...
  CT_MaxCardinalValue : Cardinal = $FFFFFFFF;

  CT_MaxPayloadSize = 255 * 500; // Max payload size in bytes
  CT_MaxContractPayloadSize = 50*1000*1000;   // Max contract payload in bytes
  CT_MaxExportTableSize = 50 * 1000; // Max size of export table in bytes

  //min fee for Save or Execute Contract
  CT_MinFeeContract = 1000;
  // 0.1000 ABEY per MB
  CT_FeePerMegatebyteInBytecode = 1000;
  // 0.01 ABEY per 1/10 MB after the 1st MB
  CT_FeePerMegabyteSubdivision = 100;
  // fee for Export table    0.1 ABEY
  CT_FeeForExportTable = 1000;
  // fee for instruction 0.0001
  // this means 1 ABEY for 10,000 instructions
  CT_FeePerInstruction = 1;
  // fee for memory usage per kb  0.0001 ABEY per KB of memory usage
  // this means 1 ABEY for 10,000 kb of memory meaning 10 mb;
  CT_FeeForMemoryUsage = 1;
  // fee for runtime usage in seconds is 0.1000 ABEY per second
  CT_FeeForRuntime = 1000;
  // fee for IO usage 0.001 for IO operation
  CT_FeeForIoUsage = 10;
  // fee for Data Segment Size per kb is 0.001 ABEY
  CT_FeeForDataSegment = 10;
  // fee per block for active blocks 0.0001
  // this means that you pay for 10,000 blocks of activy 1 ABEY
  // this is around 13.8 days
  CT_FeeForActiveBlocks = 1;
  // same for passive blocks as active blocks
  CT_FeeForPassiveBlocks = 1;
  ...
  CT_Op_SaveContract = $0B;
  CT_Op_ExecuteContract = $0C;
  CT_Op_SaveContractRefactored = $0D;
  CT_Op_ExecuteContractRefactored = $0E;
  ...
  CT_OpSubtype_SaveContractSigner         = 110;
  CT_OpSubtype_SaveContractTarget         = 111;
  CT_OpSubtype_ExecuteContratSigner       = 112;

  CT_ClientAppVersion : String = {$IFDEF PRODUCTION}'ABEY 1.2020.0428'{$ELSE}{$IFDEF TESTNET}'TESTNET 4.1'{$ELSE}{$ENDIF}{$ENDIF};

  CT_Discover_IPs = {$IFDEF PRODUCTION} '192.168.56.109' // 'n1.eu.abey.org,n2.us.abey.org'
  ...
  CT_DEFAULT_MaxVaultSnapshots = 1000;
