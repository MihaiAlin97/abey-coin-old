...
Type
  ...
  TOpMultiOperation = Class(TABEYOperation)
  private
  ...
  protected
    ...
    procedure CopyUsedPubkeySignatureFrom(SourceOperation : TABEYOperation); override;
  public
    function GetBufferForOpHash(UseProtocolV2 : Boolean): TRawBytes; override;

    function CheckSignatures(AccountTransaction : TABEYVaultTransaction; var errors : String) : Boolean;
...
begin
  ...
  if (getInfoForAllAccounts) then begin
    ...
    if (iSender>=0) then begin
      // Is a Sender account
      OperationResume.OperationTxt:='Multi SEND '+TAccountComp.FormatMoney(OperationResume.Amount * (-1))+' ABEY from ID '+TAccountComp.AccountNumberToAccountTxtNumber(Affected_account_number)+' '+changerTxt;
    end else if (iReceiver>=0) then begin
      OperationResume.OperationTxt:='Multi RECV '+TAccountComp.FormatMoney(OperationResume.Amount)+' ABEY to ID '+TAccountComp.AccountNumberToAccountTxtNumber(Affected_account_number)+' '+changerTxt;
    end else begin
      OperationResume.OperationTxt:='Multi '+changerTxt+' to ID '+TAccountComp.AccountNumberToAccountTxtNumber(Affected_account_number);
    end;
  end;
end;

procedure TOpMultiOperation.CopyUsedPubkeySignatureFrom(SourceOperation: TABEYOperation);
...
function TOpMultiOperation.CheckSignatures(AccountTransaction: TABEYVaultTransaction; var errors: String): Boolean;
var i : Integer;
  acc : TAccount;
  ophtosign : TRawBytes;
begin
  ...
  // Multioperations will not use standard TABEYOperation.IsValidECDSASignature call because will need to check more than 1 signature
  Result := False;
  // Do check it!
  Try
    ophtosign := GetDigestToSign(AccountTransaction.FreezedVault.CurrentProtocol);
    // Tx verification
...
function TOpMultiOperation.DoOperation(AccountPreviousUpdatedBlock : TAccountPreviousBlockInfo; AccountTransaction: TABEYVaultTransaction; var errors: String): Boolean;
var i,j : Integer;
  txs : TMultiOpSender;
  txr : TMultiOpReceiver;
  chi : TMultiOpChangeInfo;
...
begin
  ...
  Result := False;
  errors := '';
  if (AccountTransaction.FreezedVault.CurrentProtocol<CT_PROTOCOL_3) then begin
  ...
  for i:=low(FData.txSenders) to high(FData.txSenders) do begin
    ...
    if (txs.Account>=AccountTransaction.FreezedVault.AccountsCount) then begin
      errors := Format('Invalid sender %d',[txs.Account]);
      Exit;
    end;
    if TAccountComp.IsAccountBlockedByProtocol(txs.Account,AccountTransaction.FreezedVault.BlocksCount) then begin
  ...
  for i:=Low(FData.txReceivers) to High(FData.txReceivers) do begin
    ...
    if (txr.Account>=AccountTransaction.FreezedVault.AccountsCount) then begin
      errors := Format('Invalid receiver %d',[txr.Account]);
      Exit;
    end;
    if TAccountComp.IsAccountBlockedByProtocol(txr.Account,AccountTransaction.FreezedVault.BlocksCount) then begin
  ...
  for i:=Low(FData.changesInfo) to High(FData.changesInfo) do begin
    chi := FData.changesInfo[i];
    if (chi.Account>=AccountTransaction.FreezedVault.AccountsCount) then begin
      errors := 'Invalid change info account number';
      Exit;
    end;
    if TAccountComp.IsAccountBlockedByProtocol(chi.Account, AccountTransaction.FreezedVault.BlocksCount) then begin
    ...
    // Is locked? Protocol 2 check
    if (TAccountComp.IsAccountLocked(changer.accountInfo,AccountTransaction.FreezedVault.BlocksCount)) then begin
    ...
    If (account_name in chi.changes_type) then begin
      If (Length(chi.New_Name)>0) then begin
        If Not TABEYVault.ValidAccountName(chi.New_Name,errors) then Exit;
...
function TOpMultiOperation.IsValidSignatureBasedOnCurrentVaultState(AVaultTransaction: TABEYVaultTransaction): Boolean;
var errors : String;
begin
  Result := CheckSignatures(AVaultTransaction,errors);
end;
