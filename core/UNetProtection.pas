...
Type
  ...
  TIpInfos = Class
  private
    FThreadList : TABEYThreadList<Pointer>;
  ...
  public
    ...
    function Lock(const AIp : String; MarkAsUpdated : Boolean) : TABEYJSONObject; overload;
...
var jsonOpType, relJsonOpType, relJsonNetTransferType : TABEYJSONObject;
  lasts : TABEYJSONArray;
  iIp, i,j,k : Integer;
  list : TList<Pointer>;
  p : PIpInfo;
begin
  ...
  Try
    for iIp := 0 to list.Count-1 do begin
      ...
      for i := 0 to jsonOpType.Count-1 do begin
        if (jsonOpType.GetNameValue(i).Value is TABEYJSONObject) then begin
          relJsonOpType := TABEYJSONObject( jsonOpType.GetNameValue(i).Value );
          for j := 0 to relJsonOpType.Count-1 do begin
            if (relJsonOpType.GetNameValue(j).Value is TABEYJSONObject) then begin
              relJsonNetTransferType := TABEYJSONObject( relJsonOpType.GetNameValue(j).Value );
...
begin
  FThreadList := TABEYThreadList<Pointer>.Create(Self.ClassName);
...
var json, objinfo : TABEYJSONObject;
  arr : TABEYJSONArray;
  msg : String;
  i : Integer;
  currts : Integer;
...
function TIpInfos.Lock(const AIp: String; MarkAsUpdated: Boolean): TABEYJSONObject;
var list : TList<Pointer>;
  i : Integer;
  p : PIpInfo;
begin
...
var json, relJson : TABEYJSONObject;
  lasts : TABEYJSONArray;
  currts : Integer;
...
begin
  ...
  Try
    ...
    if Assigned(relJson) then begin
      ...
      if (DoUpdate) then begin
        // Save to last values
        relJson := TABEYJSONObject.Create;
