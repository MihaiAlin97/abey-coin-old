...
Type
  ...
  private
    ...
    FStorageLock : TABEYCriticalSection;
    ...
    Function StreamBlockRead(Stream : TStream; iBlockHeaders : Integer; BlockHeaderFirstBlock, Block : Cardinal; Operations : TABEYOperationsComp) : Boolean;
    Function StreamBlockSave(Stream : TStream; iBlockHeaders : Integer; BlockHeaderFirstBlock : Cardinal; Operations : TABEYOperationsComp) : Boolean;
  ...
  protected
    ...
    Function DoLoadBlockChain(Operations : TABEYOperationsComp; Block : Cardinal) : Boolean; override;
    Function DoSaveBlockChain(Operations : TABEYOperationsComp) : Boolean; override;
    ...
    Function LoadBankFileInfo(Const Filename : AnsiString; var VaultHeader : TABEYVaultHeader) : Boolean;
    function GetFirstBlockNumber: Int64; override;
    function GetLastBlockNumber: Int64; override;
    function DoInitialize : Boolean; override;
    Function DoCreateVaultStream(blockCount : Cardinal) : TStream; override;
  ...
  public
    ...
    Class Function GetVaultCheckpointingFileName(Const BaseDataFolder : AnsiString; block : Cardinal) : AnsiString;
...
begin
  If Not Assigned(FPendingBufferOperationsStream) then begin
    fn := GetFolder(Orphan)+PathDelim+'ABEY.pending';
...
begin
  ...
  FStorageLock := TABEYCriticalSection.Create('TFileStorage_StorageLock');
end;
...
begin
  ...
  Try
    ...
    TLog.NewLog(ltInfo,ClassName,Format('Block index %d',[_intBlockIndex]));
...
function TFileStorage.DoCreateVaultStream(blockCount: Cardinal): TStream;
var fn : TFilename;
  err : AnsiString;
begin
  Result := Nil;
  fn := GetVaultCheckpointingFileName(GetFolder(Orphan),blockCount);
  If (fn<>'') and (FileExists(fn)) then begin
    Result := TFileStream.Create(fn,fmOpenRead+fmShareDenyWrite);
  end;
  If Not Assigned(Result) then begin
    err := 'Cannot load VaultStream (block:'+IntToStr(blockCount)+') file:'+fn;
...
function TFileStorage.DoLoadBlockChain(Operations: TABEYOperationsComp; Block: Cardinal): Boolean;
Var stream : TStream;
  iBlockHeaders : Integer;
  BlockHeaderFirstBlock : Cardinal;
begin
...
function TFileStorage.DoMoveBlockChain(Start_Block: Cardinal; const DestOrphan: TOrphan; DestStorage : TStorage): Boolean;
  ...
  begin
    ...
    if SysUtils.FindFirst(GetFolder(Orphan)+PathDelim+'*.vault', FileAttrs, sr) = 0 then begin
      repeat
        if (sr.Attr and FileAttrs) = FileAttrs then begin
          ...
          TLog.NewLog(ltInfo,ClassName,'Copying Vault file '+sourcefn+' to '+destfn);
          Try
      ...
      try
        ...
        try
          ...
          while LoadBlockChainBlock(ops,b) do begin
            inc(b);
            TLog.NewLog(ltDebug,Classname,'Moving block from "'+Orphan+'" to "'+DestOrphan+'" '+TABEYOperationsComp.OperationBlockToText(ops.OperationBlock));
        ...
        // If DestOrphan is empty, then copy possible updated Vault (because, perhaps current saved Vault is from invalid blockchain)
...
var
    ...
    sbHeader, goodSbHeader : TABEYVaultHeader;
begin
  LockBlockChainStream;
  Try
    FileAttrs := faArchive;
    ...
    filename := '';
    blockscount := 0;
    if SysUtils.FindFirst(folder+PathDelim+'*.vault', FileAttrs, sr) = 0 then begin
      repeat
        if (sr.Attr and FileAttrs) = FileAttrs then begin
    ...
    if (filename<>'') then begin
      TLog.NewLog(ltinfo,Self.ClassName,'Loading Vault protocol:'+IntToStr(goodSbHeader.protocol)+' with '+inttostr(blockscount)+' blocks from file '+filename);
...
begin
  Result := true;
  bankfilename := GetVaultCheckpointingFileName(GetFolder(Orphan),Bank.BlocksCount);
  if (bankfilename<>'') then begin
    TLog.NewLog(ltInfo,ClassName,'Saving Vault blocks:'+IntToStr(Bank.BlocksCount)+' file:'+bankfilename);
    fs := TFileStream.Create(bankfilename,fmCreate);
    try
      fs.Size := 0;
      fs.Position:=0;
      if LowMemoryUsage then begin
        Bank.Vault.SaveVaultToAStream(fs,0,Bank.Vault.BlocksCount-1);
      end else begin
        ms := TMemoryStream.Create;
        try
          Bank.Vault.SaveVaultToAStream(ms,0,Bank.Vault.BlocksCount-1);
          ms.Position := 0;
          fs.CopyFrom(ms,0);
        finally
          ms.Free;
    ...
    if (Orphan='') And ((Bank.BlocksCount MOD (CT_BankToDiskEveryNBlocks*100))=0) then begin
      aux_newfilename := GetFolder('') + PathDelim+'landmark_'+ inttostr(Bank.BlocksCount)+'.vault';
      try
        {$IFDEF FPC}
      ...
      Except
        On E:Exception do begin
          TLog.NewLog(lterror,ClassName,'Exception copying extra Vault file '+aux_newfilename+' ('+E.ClassName+'):'+E.Message);
        end;
      end;
    end;
  end;
...
function TFileStorage.DoSaveBlockChain(Operations: TABEYOperationsComp): Boolean;
Var stream : TStream;
  iBlockHeaders : Integer;
  BlockHeaderFirstBlock : Cardinal;
begin
...
Const CT_VaultsToStore = 10;

class function TFileStorage.GetVaultCheckpointingFileName(const BaseDataFolder: AnsiString; block: Cardinal): AnsiString;
begin
  Result := '';
  If not ForceDirectories(BaseDataFolder) then exit;
  if TABEYVault.MustVaultBeSaved(block) then begin
    // We will store landmarking
    Result := BaseDataFolder + PathDelim+'landmark'+ inttostr((block DIV CT_BankToDiskEveryNBlocks) MOD CT_VaultsToStore)+'.vault';
  end else begin
    Result := BaseDataFolder + PathDelim+'landmark_'+inttostr(block)+'.vault';
  end;
end;
...
function TFileStorage.LoadBankFileInfo(const Filename: AnsiString; var VaultHeader : TABEYVaultHeader) : Boolean;
var fs: TFileStream;
begin
  Result := false;
  ...
  try
    fs.Position:=0;
    Result := Bank.Vault.LoadVaultStreamHeader(fs,VaultHeader);
  finally
    fs.Free;
  end;
end;
...
Var fn : TFileName;
  ...
  errors : String;
begin
  TABEYThread.ProtectEnterCriticalSection(Self,FStorageLock);
  Try
    if Not Assigned(FBlockChainStream) then begin
      if FBlockChainFileName<>'' then begin
        fn := FBlockChainFileName
      end else begin
        fn := GetFolder(Orphan)+PathDelim+'ABEY.blockchain';
      end;
...
function TFileStorage.StreamBlockRead(Stream : TStream; iBlockHeaders : Integer; BlockHeaderFirstBlock, Block : Cardinal; Operations : TABEYOperationsComp) : Boolean;
Var p : Int64;
  errors : String;
  streamFirstBlock,
  _BlockSizeC,
...
function TFileStorage.StreamBlockSave(Stream : TStream; iBlockHeaders : Integer; BlockHeaderFirstBlock : Cardinal; Operations : TABEYOperationsComp) : Boolean;
Var p : Int64;
  c : Cardinal;
...
begin
  HasUpgradedToVersion2 := SysUtils.FindFirst( GetFolder(Orphan)+PathDelim+'*.vault', faArchive, searchRec) = 0;
  FindClose(searchRec);
end;
